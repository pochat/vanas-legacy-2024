...
// Check for a form submission:
 if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	echo" ".$_SERVER['REQUEST_METHOD']."  <script>alert('paso1.');</script>";
	...
	if (isset($_POST['stripeToken'])) {
        ...
        echo"   <script>alert('paso2. recibe token $token');</script>";
  ...
  $email = "gflores2702@gmail.com";  
	...
	if (empty($errors)) {
	   ...
	   echo"<script>alert('pasa a validar ningun error.');</script>";
	   ##aqui muere
    ...
    // Include the Stripe library:
      ...
      require_once('Stripe/Stripe/init.php');
		...
		try {
			 ...
			 echo"<script>alert('entra rcecibir keys.');</script>";
      ...
      */
	  echo"<script>alert('entra a realizar pago.');</script>";
	  
	  
      $charge = \Stripe\Charge::create(array(
        "amount" => $amount,
        "currency" => "USD",
        "description" => "Prueba live",
        //"receipt_email" => "gflores@loomtek.com.mx",
        "source" => $token,
      ));
      
	  echo"<script> alert('se realizo el pago correctamente'); </script>";
