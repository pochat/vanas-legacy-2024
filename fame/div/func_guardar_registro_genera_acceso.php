...
//echo"<script>alert('1');</script>";
 ...
 $Query="SELECT ds_first_name, ds_last_name, ds_email, fl_invitado_por_instituto, fl_usu_invita, fl_pais FROM k_envio_email_reg_selfp WHERE fl_envio_correo=$fl_envio_correo ";
 ...
 $Queryp="SELECT COUNT(*) FROM k_usuario_programa WHERE fl_programa_sp=$fl_programa_sp AND fl_usuario_sp=$fl_invitado_por ";
 $rowb=RecuperaValor($Queryp); 
 $existe=$rowb[0];
	 ...
	 if(empty($existe)) {
		     ...
		     if($no_email_confirmados== $requisito_email ){#Ya existe uno asi , que esta cueta a activarse en total ya seran dos, lo que cumple para poder desbloquera curso.NOTA: los siguientes email que se activen ya no tendran validez, para debloquear curso.
				...
				if(empty($no_reg)){
						...
						$Query.="VALUES($fl_invitado_por,$fl_programa_sp,0,'0','0','RD','0',0,$fl_maestro,'0',CURRENT_TIMESTAMP, 0)";
					    ...
					    ";
                           ...
                           #Se genera un email de confirmacion al alumno que le desbloquearon curso. 
                           $ds_encabezado = genera_documento_sp($fl_invitado_por,1,161,$fl_programa_sp);
                           $ds_cuerpo = genera_documento_sp($fl_invitado_por,2,161,$fl_programa_sp);
                           $ds_pie = genera_documento_sp($fl_invitado_por,3,161,$fl_programa_sp);
                           $ds_contenido=$ds_encabezado.$ds_cuerpo.$ds_pie;
                           
						   $no_dias=ObtenConfiguracion(127);
						   
                           $ds_contenido = str_replace("#fame_pg_name#", $nb_programa_desbloquear, $ds_contenido); 
						   $ds_contenido = str_replace("#fame_number_of_days#",$no_dias, $ds_contenido); 						   
 ...
 #Recuperamos quien se registro:
 $Query="SELECT fl_usuario,fl_usuario FROM k_envio_email_reg_selfp WHERE fl_envio_correo=$fl_envio_correo";
 $row=RecuperaValor($Query);
 $fl_usuario=$row[0];
 
 #CREAOS EL USUARIO Y PASSOWR PARA ACCEDER AL SISTEMA. y se activa su cuenta
 // Inserta el usuario
 $Query  = "INSERT INTO c_usuario(fl_usuario, ds_login, ds_password, ds_alias, cl_sesion, fg_activo, fe_alta, no_accesos, ";
 $Query .= "ds_nombres, ds_apaterno, ds_email, ";
 if($fl_perfil_sp==PFL_ESTUDIANTE_SELF)
  $Query.="fg_genero,fe_nacimiento, ";
 $Query.="fl_perfil,fl_perfil_sp,fl_instituto, fl_usu_invita, flag) ";
 $Query .= "VALUES('$fl_usuario', '$ds_login', '".sha256($ds_pass)."','$ds_alias', '$cl_sesion_nueva', '$fg_activo', CURRENT_TIMESTAMP, 0, ";
 $Query .= "'$ds_nombres', '$ds_apaterno','$ds_email', ";
 if($fl_perfil_sp==PFL_ESTUDIANTE_SELF)
  $Query .= "'$fg_tipo_sexo',$fe_nacimiento, ";
 $Query .=" $fl_perfil,$fl_perfil_sp,$fl_instituto, $fl_usu_invita, 0) ";
 ...
 $Query .= "ds_login = '$ds_login',
            ...
            ds_email = '$ds_email',";
  if($fl_perfil_sp==PFL_ESTUDIANTE_SELF)
    $Query .= "fg_genero = '$fg_tipo_sexo',
             fe_nacimiento = $fe_nacimiento, ";
  $Query .= "fl_perfil = '$fl_perfil',
            fg_system = 'F', 
            fl_perfil_sp = '$fl_perfil_sp',
            fl_instituto= $fl_instituto,
            fl_usu_invita=$fl_usu_invita,
            flag=0 ;";


  EjecutaQuery($Query);
  
  $fl_usuario_sp = $fl_usuario;
 ...
 if($fg_desbloquear_curso==1){
     ...
     EjecutaQuery("UPDATE c_usuario SET token='$token', fg_b2c='1',fl_instituto=$fl_instituto,fg_system='F' WHERE fl_usuario=$fl_usuario_sp ");
     


     
     #Envia email de bienvenida a FAME B2C.
     #Se genera un email de confirmacion al alumno que le desbloquearon curso. 
     $ds_encabezado = genera_documento_sp($fl_invitado_por,1,162,$fl_programa_sp);
     $ds_cuerpo = genera_documento_sp($fl_invitado_por,2,162,$fl_programa_sp);
     $ds_pie = genera_documento_sp($fl_invitado_por,3,162,$fl_programa_sp);
     $ds_contenido=$ds_encabezado.$ds_cuerpo.$ds_pie;
     #Sustituimos datos.
     $ds_contenido = str_replace("#fame_admin_user#", $ds_nombres." ".$ds_apaterno, $ds_contenido); 
     $ds_contenido = str_replace("#fame_link_login_fame#", ObtenConfiguracion(116), $ds_contenido);

     $message  = $ds_contenido;
     $message = utf8_decode(str_ascii(str_uso_normal($message)));
     #Envia email de notificcion al usuario
     $mail = EnviaMailHTML($nb_nombre_dos, $ds_email_de_quien_envia_mensaje, $ds_email, $ds_titulo, $message, $bcc);
 ...
 if($fl_perfil_sp==PFL_ESTUDIANTE_SELF){
	...
	if($fg_desbloquear_curso){
		...
		#Verifica que no exista el registro.
		$Query="SELECT COUNT(*) FROM k_usuario_programa WHERE fl_usuario_sp=$fl_usuario_sp AND fl_programa_sp=33 ";
		$rw=RecuperaValor($Query);
		$no_reg=$rw[0];
		...
		$Query.="VALUES($fl_usuario_sp,33,0,'0','0','RD','0',0,$fl_usu_invita,'0',CURRENT_TIMESTAMP)";
		$fl_usu_pro=EjecutaInsert($Query);
 ...
 if(empty($fl_pais)){
      $Query="SELECT fl_pais FROM c_instituto WHERE fl_instituto=$fl_instituto "; $row=RecuperaValor($Query); $fl_pais=$row['fl_pais'];
 }
#Se genera su direccion , por el momento solo es su fl_sp.
  ...
  if(!empty($row0[0])){
    EjecutaQuery("UPDATE k_usu_direccion_sp SET fl_usuario_sp=$fl_usuario_sp ,fl_pais=$fl_pais WHERE fl_usuario_sp=$fl_envio_correo");
  }
  else{
    $Query ="INSERT INTO k_usu_direccion_sp ( fl_usuario_sp,fl_pais ) ";
    $Query .="VALUES ($fl_usuario_sp,$fl_pais ) ";
 ...
 #Generamos su token
 $token=sha256($fl_usuario_sp);
 $ds_password = sha256($ds_pass);
 #actualizamos su ID DS_LOGIN
 $Query="UPDATE c_usuario SET token='$token', ds_login='$ds_login3',fg_system='F',ds_password = '".$ds_password."' WHERE fl_usuario=$fl_usuario_sp ";
 EjecutaQuery($Query);
 ...
 if($fl_perfil_sp==PFL_ESTUDIANTE_SELF){
      ...
      #Verifica si es un instituto b2c disminuye las licencias. 
      #Verifica que el instituto sea b2c.
      $Query="SELECT fg_b2c,no_tot_licencias_b2c,no_licenias_usadas_b2c,no_licencias_disponibles_b2c FROM c_instituto WHERE fl_instituto=$fl_instituto ";
      $row=RecuperaValor($Query);
      $fg_b2c=$row['fg_b2c'];
      $no_total_licencias=$row['no_tot_licencias_b2c'];
      $no_usadas=$row['no_licenias_usadas_b2c'];
      $no_disponibles=$row['no_licencias_disponibles_b2c'];
      $no_usadas=$no_usadas+1;
      $no_disponibles=$no_disponibles-1;
      if($fg_b2c==1){

          EjecutaQuery("UPDATE c_instituto SET no_licencias_disponibles_b2c=$no_usadas,no_licencias_disponibles_b2c= $no_disponibles WHERE fl_instituto=$fl_instituto ");
