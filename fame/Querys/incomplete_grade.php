					...
					if($tot_entregables == 0){
					 ...
					 if(!empty($fl_promedio_semana)) {
						...
						echo '
						{
						  ...
						  "name": "<a href=\'#site/desktop.php?student='.$fl_alumno.'&week='.$no_semana.'&tab='.(!empty($nb_tab)?$nb_tab:NULL).'&fl_programa='.$fl_programa_sp.'&t=1\' rel=\'tooltip\' data-placement=\'top\' data-html=\'true\' data-original-title=\'View '.$ds_nombre.' desktop\'>'.$ds_nombre.'</a><br><br>'.$ds_status.'",
						  "nada": "<span hidden>'.(!empty($fe_calificado)?$fe_calificado:NULL).'</span>&nbsp;",
						  ...
						  "trabajos": " '.$tot_entrega.'  <a href=\'#site/desktop.php?student='.$fl_alumno.'&week='.$no_semana.'&tab='.(!empty($nb_tab)?$nb_tab:NULL).'&fl_programa='.$fl_programa_sp.'&t=1\'>'.(!empty($ds_tipo)?$ds_tipo:NULL).''.(!empty($ds_orden)?$ds_orden:NULL).'</a>&nbsp;&nbsp;'.(!empty($ds_coment)?$ds_coment:NULL).'<br> ",
