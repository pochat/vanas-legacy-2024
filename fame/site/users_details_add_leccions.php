<?php 
  ...
  $p=RecibeParametroNumerico('p',true,False);
  
   $fl_perfil_usr=ObtenPerfilUsuario($clave);

 //if(!empty($p)){ 20/may/2019, se coemnta ya que no se ve necesario, siempre se buscara atarves del fl_usu_pro  
 if($fl_perfil_usr<>PFL_MAESTRO_SELF){ 
 ...
 // }
  ...
  # indica que aun no ha confirmado el usuario
  ...
  $row=RecuperaValor($Query); 
  $fg_permite_ver_historial=$row[0];
        ...
        <div class="tab-pane fade"  id="assigment_grade">
			...
			<?php if($fg_permite_ver_historial==1){ ?> 
                    ...
                    $Query3="SELECT A.fl_alumno,A.fl_leccion_sp,A.fl_promedio_semana,C.nb_programa,B.ds_titulo,B.no_valor_rubric,B.no_semana,D.cl_calificacion,D.no_equivalencia 
                            ...
                            $total=$sum_porcentaje/$total_reg;
