 ...
 if(!ValidaPermisoSelf(FUNC_SELF)) {
  ...
  if(empty($fg_idiomas)){
    ...
    if(!empty($fl_leccion)){    
      ...
      $ruta = $_SERVER[DOCUMENT_ROOT]."/vanas_videos/fame/lessons/video_".$fl_leccion."/";    
      ...
      if($accion=="D" || $accion=="AD"){
        ...
        if(!ExisteEnTabla('k_idioma_video', 'fl_leccion_sp', $fl_leccion, 'fl_idioma', $fl_idioma, true)){    
          ...
          $message = ObtenEtiqueta(2024)."&nbsp;".$nb_idioma;
            ...
            $Query = "UPDATE k_idioma_video SET ds_language='".str_texto($ds_language)."', fg_activo='1' WHERE fl_leccion_sp=$fl_leccion AND fl_idioma=$fl_idioma ";
  ...
  # Envviamos el layout actulaizado de los idiomas
    ...
    $idiomas = "<br><br><ul style='padding-left: 15px; padding-top:10px; color: darkgrey;'>";    
    ...
    for($i=1;$rowl=RecuperaRegistro($rs);$i++){
      ...
      <li class='message no-margin' style='padding-top:2px;'>
        <i class='fa ".$eye." cursor-pointer' rel='tooltip' data-placement='top' data-original-title='".$lbl_eye."' onclick='Languages(".$fl_leccion.", ".$fl_idioma_bd.", \"AD\");'></i> 
        <a href='javascript:void(0);'   onclick='MuestraModalLang(1,$fl_idioma_bd,$fl_leccion);' data-toggle='modal' data-target='#LanguageModal' rel='tooltip' data-placement='".$pla2."' data-original-title='Editar'>".$ds_language."</a> 
