...
<div style="width: 218px; right: 0px; display: block; padding:0px 0px 0px 145px;" outline="0" class="ui-widget ui-chatbox">
          <a href="index.php#site/Imedia_frm.php" class="btn btn-primary btn-circle btn-xl" title="Add record" style="color:#FFFFFF;height: 59px;
      ...
      SectionIni();
       ...
       // ObtenEtiqueta(1120), ObtenEtiqueta(1121), ObtenEtiqueta(1122), ObtenEtiqueta(1909), ObtenEtiqueta(1816),ObtenEtiqueta(2310),ObtenEtiqueta(2328));
        //$val_btn = array(ADD_STD, IMP_STD, ADD_MAE, IMP_MAE, ASG_GROUP, ASG_COURSE, DESASIGNAR_COURSE, ACTIVE, DESACTIVE, DELETE, ASSESSMENT, ASSIGN_MYSELF,RESEND_EMAIL_INVITATION,ASG_TEACHER);
          ArticleIni("col-xs-12 col-sm-12 col-md-12 col-lg-12", "students2", "fa-table", ObtenEtiqueta(2580), true, true, false, false, false, ObtenEtiqueta(1074), "default", (isset($opt_btn)?$opt_btn:NULL), (isset($val_btn)?$val_btn:NULL), (isset($desc_btn)?$desc_btn:NULL));
          # Muestra Inicio de la tabla
          $titulos = array('',''.ObtenEtiqueta(360).'', ''.ObtenEtiqueta(1230).'',''.ObtenEtiqueta(1234).'',''.ObtenEtiqueta(1235).'', ''.ObtenEtiqueta(1236).'',
          ''.ObtenEtiqueta(1237).'', ''.ObtenEtiqueta(1238).'',''.ObtenEtiqueta(1266).'',''.ObtenEtiqueta(1328).'',''.ObtenEtiqueta(1329).'');
          ...
          CampoOculto('tot_reg', ($tot_reg??NULL));
  ...
  var pagefunction = function() {
      ...
      function format (d) {
          ...
          return '<table cellpadding="5" cellspacing="0" border="0" class="table table-hover table-condensed" width="100%">' +
              '<tr>'+
                '<td><?php echo ObtenEtiqueta(1234); ?>: <b>'+d['title']+'</b><br><?php echo ObtenEtiqueta(1297); ?>: <b>'+d['outcome']+'</b></td>'+
              '</tr>'+
          '</table>';
      }
        ...
        if(processing == false)
          $("vanas_loader").hide();
	        ...
	        "columns": [
                ...
                { "data": "valor_rubric" },
                { "data": "action" },
                { "visible": false, "data": "outcome", "visible": false, "data": "title"}
	        ],
	        "order": [[1, 'asc']],
             ...
             var fg_curso_asignado = result.fg_curso_asignado;
				...
				if(resultado==1){											 
				   ...
				   $('#tbl_users').DataTable().ajax.reload();
				    ...
				    var etq = "";
				    if (fg_curso_asignado == 1) {
				        var etq = "<?php echo ObtenEtiqueta(2630);?>";
				    }
					 $.smallBox({
					   title : "<i class=\"fa fa-times-circle-o\" aria-hidden=\"true\"></i> <?php echo ObtenEtiqueta(2627); ?><br>"+etq+" ",
					   //content : "Lorem ipsum dolor sit amet, test consectetur adipisicing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam",
					   color : "#a90329",
					   timeout: 7000,
					   iconSmall : "fa fa-check ",
