<?php 
  ...
  $Query  = "SELECT fl_entregable_sp, nb_archivo ";
  $Query .= "FROM k_gallery_post_sp ";
  $Query .= "WHERE fl_gallery_post_sp=$fl_gallery_post AND fl_usuario=$fl_usuario";
  $row = RecuperaValor($Query);
  $fl_entregable_sp = $row[0];
  $nb_archivo = $row[1];
  ...
  if(empty($fl_entregable_sp)) {
  	...
  	if($ext == 'jpg') {
  		...
  		if($file_name != $board_default AND $file_name != $board_video_default) {
	  		$path = PATH_SELF_UPLOADS_F."/gallery";
	  		$thumbs = $path."/thumbs";
	  ...
	  EjecutaQuery("DELETE FROM k_gallery_comment_sp WHERE fl_gallery_post_sp=$fl_gallery_post");
	  # Delete the post
	  EjecutaQuery("DELETE FROM k_gallery_post_sp WHERE fl_gallery_post_sp=$fl_gallery_post");
