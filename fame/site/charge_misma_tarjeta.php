<?php
  ...
  #Recupeamor al customer 
  $Query="SELECT id_cliente_stripe FROM k_alumno_tarjeta WHERE fl_usuario=$fl_usuario ";
  $row=RecuperaValor($Query);
  $id_customer_db=str_texto($row['id_cliente_stripe']);
  
  $customer = \Stripe\Customer::retrieve($id_customer_db);
  ...
  if(!empty($fl_programa_sp)){
    ...
    $email_cliente =  $row[2];
            ...
            if($fg_plan_curso){ #Quiere decir que pago un plan de liberacion de cursos.
                       ...
                       $Query="INSERT INTO k_admin_pagos_alumno (fl_current_plan_alumno,mn_total,fe_periodo_inicial,fe_periodo_final,fg_pagado,fe_pago,ds_descripcion,id_invoice_stripe,id_pago_stripe,mn_subtotal,mn_tax,mn_descuento,mn_porcentaje_cupon,fg_tipo_descuento)";
                       $Query.="VALUES($fl_current_plan,$mn_total_plan,'$fe_actual','$fe_periodo_final','1',CURRENT_TIMESTAMP,'$nb_plan','$id_invoice','$id_charge',$mn_monto_normal,$mn_tax,$mn_porcentaje_descuento,$mn_porcentaje_cupon, '$fg_tipo_descuento' )";
