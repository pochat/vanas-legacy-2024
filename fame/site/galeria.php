...
<!-- Board tool bar -->
<div id="board-header" class="hidden">
	<div class="pull-left">
...
<script type="text/javascript">
	...
	$(document).ready(function(){
		// Set classmate flag
		boardControllerGaleria.setClassmate(classmate);
		...
		boardControllerGaleria.setupFilterDropdown(dropdownPrograms, programs, "Programs");

		// If there is a default boardPost, open up the modal
		if(boardPost){
			boardControllerGaleria.setSelectedPost(boardPost);
			modalItemContainer.on("show.bs.modal", function(){        
			  ...
			  boardControllerGaleria.requestComments(modalCommentsContainer);
			});
		...
		boardControllerGaleria.requestItemsboard(container,<?php echo $fl_usuario;?>,<?php echo $fl_usuario_actual;?>);
		...
		dropdownPrograms.on("click", "li", function(){
			...
			boardControllerGaleria.emptyContainer(container);
			...
			boardControllerGaleria.setSelectedFilter(selectedFilter);
			boardControllerGaleria.requestItemsboard(container,<?php echo $fl_usuario;?>,<?php echo $fl_usuario_actual;?>);
		});
		...
		buttonMyClass.on("click", function(){
			...
			// Present the board with the new selected filter
			boardControllerGaleria.setClassmate('on');
			boardControllerGaleria.requestItemsboard(container,<?php echo $fl_usuario;?>,<?php echo $fl_usuario_actual;?>);
		});
		...
		buttonMyPosts.on("click", function(){
			...
			// Present the board with the new selected filter
			boardControllerGaleria.setMyPosts('on');
			boardControllerGaleria.requestItemsboard(container,<?php echo $fl_usuario;?>,<?php echo $fl_usuario_actual;?>);
		});
		...
		container.on("click", "a[data-toggle='modal']", function(){
			...
			boardControllerGaleria.setSelectedPost(post);
		});

		// Submiting a new post
		postButtonSubmit.on("click", function(){
			boardControllerGaleria.submitPost(postContainer);
			return false;
		});
		...
		container.on('click', '.item > .delete-me', function(event){
			var post = $(this).data('selected-post');
			boardControllerGaleria.deletePost(container, event, post);
			return false;
		});
...
//      alert('gabriel');
//			boardControllerGaleria.submitComment(modalItemContainer);
//			return false;
//		});
		...
		$(window).on('scroll.infinite', function (){
			if($(window).scrollTop() == $(document).height() - $(window).height()) {
		  	boardControllerGaleria.requestItemsboard(container,<?php echo $fl_usuario?>);
		  }
		});
