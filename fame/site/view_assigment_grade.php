   ...
   <?php      
      SectionIni();
        ...
        ArticleIni("col-xs-12 col-sm-12 col-md-12 col-lg-12", "students", "fa-table", ObtenEtiqueta(1921), true, true, false, false, false, ObtenEtiqueta(1074), "default", $opt_btn, $val_btn, $desc_btn);
          ...
          MuestraTablaIni("tbl_assesment", "display projects-table table table-striped table-bordered table-hover", "100%", $titulos,false);          
  ...
  /** INICIO DE SCRIPT PARA DATATABLE **/
	var pagefunction = function() {
		...
		var table = $('#tbl_assesment').on('processing.dt', function (e, settings, processing) {
        $('#datatable_fixed_column_processing').css( 'display', processing ? 'block' : 'none' );        
        $("#vanas_loader").show();
        if(processing == false)
          $("vanas_loader").hide();
        }).DataTable( {
	        "ajax": "Querys/assignment_grade.php",
	        "bDestroy": true,
			"stateSave": true,
	        "iDisplayLength": 10,
	        "scrollX": true,
		...
		$('#tbl_assesment tbody').on('click', 'td.details-control', function () {
   ...
   function ResetFilter(){
	    ...
	    $("#tbl_assesment").DataTable().search("").draw();  //limpiar el restet
...
<script>
    ...
    $('#tbl_assesment_filter').append(button);
