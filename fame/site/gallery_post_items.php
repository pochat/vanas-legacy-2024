<?php
  ...
  $Queryp .= "cl_clasificacion_grado, fl_instituto, fl_pais, nb_programa".$sufix.", fl_perfil_sp, fl_grado, edad, fame, fe_post, fl_programa_sp FROM ( ";
    ...
    $Queryp .= "ELSE 0 END cl_clasificacion_grado, c.fl_instituto, ''fl_pais, (SELECT p.nb_programa".$sufix." FROM c_programa_sp p WHERE p.fl_programa_sp=d.fl_programa_sp ) nb_programa".$sufix." , c.fl_perfil_sp, fe_post, ";
    $Queryp .= "TIMESTAMPDIFF(YEAR,c.fe_nacimiento,CURDATE()) AS edad, '1' fame, d.fl_programa_sp,fg_educational ";
    $Queryp .= "FROM k_gallery_post_sp a  ";
    //$Queryp .= "LEFT JOIN k_gallery_comment_sp kgcsp ON(kgcsp.fl_gallery_post_sp=a.fl_gallery_post_sp) ";//SE ECOMENTA POR OPTIMIZACION NO SE REQUERIA ESTE JOIN
    $Queryp .= "JOIN c_usuario c ON (a.fl_usuario=c.fl_usuario) ";
    //$Queryp .= "JOIN k_programa_detalle_sp d ON (d.fl_programa_sp =a.fl_programa_sp OR a.fl_programa_sp=0) ";
    $Queryp .= "JOIN k_programa_detalle_sp d ON (d.fl_programa_sp =a.fl_programa_sp ) ";
    //$Queryp .= "JOIN c_programa_sp p ON(p.fl_programa_sp=d.fl_programa_sp) ";
    //$Queryp .= "LEFT JOIN c_administrador_sp ad ON( ad.fl_adm_sp=c.fl_usuario) ";//SE ECOMENTA POR OPTIMIZACION NO SE REQUERIA ESTE JOIN
    //$Queryp .= "LEFT JOIN c_maestro_sp ma ON( ma.fl_maestro_sp=c.fl_usuario) ";//SE ECOMENTA POR OPTIMIZACION NO SE REQUERIA ESTE JOIN
    $Queryp .= "LEFT JOIN c_alumno_sp al ON( al.fl_alumno_sp=c.fl_usuario) ";
    //$Queryp .= "JOIN k_usu_direccion_sp usd ON(usd.fl_usuario_sp=c.fl_usuario) ";//SE ECOMENTA POR OPTIMIZACION NO SE REQUERIA ESTE JOIN
    ...
    # Posts vanas
    if(empty($fl_programa_sp) || empty($my_posts))
      $Queryp .= "UNION (".VanasBoard($fl_usuario, $fl_instituto, $index_end, $index).")";
  $Queryp .= ") as MainPost WHERE 1=1 "; 
  if(!empty($fl_programa_sp))
    $Queryp .= "AND fl_programa_sp=".$fl_programa_sp." ";
  if(!empty($my_posts))
    $Queryp .= "AND fl_usuario=".$fl_usuario." ";
  # INICIAMOS CON LA PRIVACIDAD
  # Get privacity gender
  $privacity_gender = GetPrivacityGenderInst($fl_instituto, $fl_usuario);
  if(!empty($privacity_gender)){
    $Queryp .= "AND fg_genero='".$privacity_gender."' ";
  }
  # Get privacity grade
  $privacity_grade = GetPrivacityGradesInst($fl_instituto, $fl_usuario);
  if(!empty($privacity_grade)){
    $Queryp .= "AND ".substr($privacity_grade, 3);
  }
  # Get privacity educatonal 
  $privacity_educational = GetPrivacityEducationalInst($fl_instituto, $fl_usuario);
  if(!empty($privacity_educational)){
    $Queryp .= $privacity_educational;
  }
  # Get privacity international
  $privacity_international = GetPrivacityInternationalInst($fl_instituto, $fl_usuario);
