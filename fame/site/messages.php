					...
					<div class="col-xs-12 col-sm-7 col-md-9">
						...
						<form id='comment-form' method='POST' class='panel-footer' onsubmit='return false;' style="border-right:1px solid #E3E3E3;">
							<div class='input-group'>
								<input type='text' id='message' name='message' class='form-control' placeholder='Write a reply...'>
					      <span class='input-group-btn'>
					        <!--<span id='message-submit' class='btn btn-default btn-primary' style='display:inline;'>Send</span>-->
					        <input  type="submit" id='message-submit' class='btn btn-default btn-primary' style='display:inline;' value="<?php echo ObtenEtiqueta(1898); ?>">
					      </span>
							</div>
						</form>
					</div>
...
<script type="text/javascript">
	...
	recentUsers = <?php GetChatUsers($fl_usuario,1); ?>;
	...
	$(document).ready(function(){
		...
		if(userDest == 0){ 
      var size_chat = recentUsers.size.total; 
      if(size_chat>0)
        userDest = recentUsers.user0.id;       
    }
		messagesController.setSelectedUser(userDest);
		messagesController.requestMessages(messageContainer, 0);
		...
		$("#message-submit").on("click", function(){
			messagesController.submitReply($("#message"), userOri, userOriAvatar, 0);      
		});
	});
</script>