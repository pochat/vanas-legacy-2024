...
$(document).ready(function() {
  ...
  if($accion == 0){
    # Inserta registro
    EjecutaQuery("INSERT INTO k_criterio_programa_fame (fl_criterio, fl_programa_sp, no_valor) VALUES ($fl_criterio_i, $clave, NULL)");
  }else{  
    # Inserta registro
    EjecutaQuery("DELETE FROM k_criterio_programa_fame WHERE fl_criterio = $accion AND fl_programa_sp = $clave");
  }
...
?>    
                  ...
                  <tr>
                    ...
                    for($m=1;$rowm=RecuperaRegistro($rm);$m++){
                        ...
                        echo "<td  width='12%'>$ds_calificacion1<br><small class='text-muted'><br>
                          ...
                          <small class='text-muted' style='font-size: 100%;'><i>$ds_descripcion1</i></small>              
                          </div>
                        </small></td>";
                      }
                    ?>
                    <td  width="15%">
                      <div class="widget-body"  style="padding-top:20px; vertical-align: middle; font: bold 40px Arial; text-align: center; ">
                        <div id="user_<?php echo $fl_criterio; ?>"  style="clear: both">
                          <a href="site/form-x-editable.html#" id="username_<?php echo $fl_criterio; ?>" data-placement="left" data-type="text" onkeyup="ValidaNumeros<?php echo $fl_criterio;?>(this.value);" data-pk="<?php echo $fl_criterio; ?>" data-original-title="&nbsp;"><?php echo $no_valor; ?></a>%
                        </div>
                      </div>
                    </td>
                  </tr>
   ...
   <script src="../../fame/js/plugin/x-editable/moment.min.js"></script>
    ...
    <script type="text/javascript">
	  ...
	  //vuelvo a ejecutra el editable solo asi funciona.
	   //editables
        $('#username_<?php echo $fl_criterio; ?>').editable({
            url: 'site/suma_criterios.php',
            type: 'text',
	  ...
	  function ValidaNumeros<?php echo $fl_criterio;?>(value){
		  
		  var regex = [0-9];
		  if(!regex.test(value)) {
			  alert('no es valido');
			  $('#username_<?php echo $fl_criterio; ?>').value('');
		  }else{
