   ...
   <?php      
      SectionIni();
        ...
        ArticleIni("col-xs-12 col-sm-12 col-md-12 col-lg-12", "students", "fa-table", ObtenEtiqueta(1921), true, true, false, false, false, ObtenEtiqueta(1074), "default", $opt_btn, $val_btn, $desc_btn);
          ...
          $titulos = array( ObtenEtiqueta(1795),  ObtenEtiqueta(1796), ObtenEtiqueta(1797), ObtenEtiqueta(1798),ObtenEtiqueta(1799),ObtenEtiqueta(2380));
          MuestraTablaIni("tbl_history", "display projects-table table table-striped table-bordered table-hover", "100%", $titulos,false);          
  ...
  /** INICIO DE SCRIPT PARA DATATABLE **/
	var pagefunction = function() {
		...
		var table = $('#tbl_history').on('processing.dt', function (e, settings, processing) {
        $('#datatable_fixed_column_processing').css( 'display', processing ? 'block' : 'none' );        
        $("#vanas_loader").show();
        if(processing == false)
          $("vanas_loader").hide();
        }).DataTable( {
	        "ajax": "Querys/history_grade.php",
	        "bDestroy": true,
			"stateSave": true,
	        "iDisplayLength": 10,
	        "scrollX": true,
	...
	function AsignarCalificacion(fl_alumno, fl_leccion_sp, fl_programa_sp, no_semana, fl_entrega_semanal_sp, fl_teac,fg_tab_assigment) {
	    ...
	    var fl_entrega_semanal_sp = fl_entrega_semanal_sp;
			...
			// alert("fl_alumno:"+fl_alumno+",fl_leccion_sp:"+fl_leccion_sp+",fl_programa_sp:"+fl_programa_sp+"no_semana:"+no_semana+",fl_entrega_semanal_sp:"+fl_entrega_semanal_sp);
	        $.ajax({
	            type: 'POST',
	            ...
	            data: 'fl_alumno='+fl_alumno+
                       ...
                       '&fl_teacher='+fl_teac+
					   '&fg_tab_assigment='+fg_tab_assigment,
    ...
    function ResetCalificacion(fl_alumno, fl_leccion_sp, fl_programa_sp, no_semana, fl_entrega_semanal_sp){
   ...
   function ResetFilter(){
	    ...
	    $("#tbl_history").DataTable().search("").draw();  //limpiar el restet
...
<script>
    ...
    $('#tbl_history_filter').append(button);
