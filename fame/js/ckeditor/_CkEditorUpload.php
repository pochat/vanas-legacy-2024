...
class CkEditorUpload{
    ...
    function upload($file, $destination, $name = NULL, $rules = NULL, $allowed = NULL) {
        ...
        if (!is_null($allowed)) {
            ...
            $this->_allowed = array('jpg', 'jpeg', 'gif', 'png','pdf', 'JPG', 'JPEG', 'GIF', 'PNG', 'PDF');
        }
    ...
    function image($file, $type, $size, $output = NULL, $quality = NULL) {
        ...
        // -- check output
        if ($output != 'jpg' && $output != 'png' && $output != 'gif' && $output != 'pdf') {
        ...
        switch ($output) {
            ...
            case 'gif':
                ...
                break;
                case 'pdf':
                $write = imagegif($dstImg, $this->_name . ".pdf", $quality);
                break;
        }
    ...
    function uniquename($file) {
        ...
        //comentado para MVT para que se sobreescriba con el mismo nombre la foto a fuerzas sin importar validancias
        //	return $dir.'/'.$file.$ext;
    }
