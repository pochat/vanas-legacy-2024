<?php
  ...
  if(!empty($fl_term_pago) AND !empty($no_pago)){
    ...
    $total_x_invoice = $mn_tuition / $no_invoices;
    ...
    $porc_interes = $mn_x_paid / $mn_tuition;
    ...
    if(($fl_alumno==1535)&&($no_pago==1)){
        $pago_normal_x_mes = $mn_x_due/$no_meses_op;
    }
    if($fl_alumno==2405){
        $pago_normal_x_mes = $mn_x_due/3;
    }
...
#damos a conocer si el estudiantes es de canada
  ...
  */
  ...
  $htmlcontent .='
    ...
    if(!empty($fl_term_pago) AND !empty($no_pago)){
      ...
      for($i=0;$i<=$no_meses_op-1;$i++){
        ...
        if($i==$no_meses_op)
        {
            ...
            /*if (($no_invoices > 0) && ($mn_costs>0) ) {

                $pago_normal_x_mes = round(($pago_normal_x_mes - $mn_cost_x_mes), 2);
    ...
    /*  if($mn_costs>0 ){
		  #08-ago-2019 ya no se mostrara. se reactiva 2023 se vuelve a mostrar, hbo telefono descompuesto se vuelve a ocultar por que existira otro recibo fame fee. aparte
    ...
    */
      ...
      if($cl_pais==38 AND (!empty($fg_tax_rate) || empty($fg_tax_rate))){
        ...
        $htmlcontent.='
        <br /> Total: </td>
          <td align="right">'.number_format($subtotal,2,'.',',').'<br />'.number_format($mn_tax_paypal_appfee,2,'.',',').'  ';
        ...
        $htmlcontent.=' <br />'.number_format(round($total+$convenience_fee_app_fee),2,'.',',').'</td>
        </tr>';
      }
      else{
        $htmlcontent .='
        <tr style="text-align:right; font-size:12px;">
          ...
          <td align="right">'.number_format(round($mn_pagado_app),2,'.',',').'</td>
        </tr>';
      }
    }
  $htmlcontent .='
...
$nb_programa_pdf = str_replace(" ","_",$nb_programa);

  $nombre_archivo = 'Invoice '.$ds_fname.' '.$ds_lname.' '.$Date.'_'.$nb_programa_pdf.'_'. $additional_label.'_'.'.pdf';
