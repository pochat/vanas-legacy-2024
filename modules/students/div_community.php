  ...
  if(empty($category) OR $category == 'T') {
    ...
    for($i = 0; $i < $tot_renglones; $i++) {
      for($j = 0; $j < $tot_columnas; $j++) {
        ...
        if(!empty($row[0])) {
          $fl_maestro[$j] = $row[0];
          if($fl_maestro[$j] <> $fl_mi_maestro) {
            $clase[$j] = 'comm_teachers';
            $ds_perfil[$j] = 'Teacher';
          }
          else {
            $clase[$j] = 'comm_my_teacher';
            $ds_perfil[$j] = 'My Teacher';
          }
  ...
  if(empty($category) OR $category == 'S') {
    ...
    for($i = 0; $i < $tot_renglones; $i++) {
      for($j = 0; $j < $tot_columnas; $j++) {
        ...
        if(!empty($row[0])) {
          $fl_alumno[$j] = $row[0];
          if($fl_alumno[$j] <> $fl_usuario) {
            $fl_grupo = ObtenGrupoAlumno($fl_alumno[$j]);
