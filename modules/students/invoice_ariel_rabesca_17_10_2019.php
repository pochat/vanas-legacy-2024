<?php
  ...
  //if(empty($destino)) {
    ...
    $row = RecuperaValor("SELECT cl_sesion, nb_grupo FROM c_usuario a, k_alumno_grupo b, c_grupo c WHERE fl_usuario=$fl_alumno AND a.fl_usuario=b.fl_alumno AND b.fl_grupo=c.fl_grupo");
    ...
    $row = RecuperaValor("SELECT cl_sesion, fg_inscrito FROM c_sesion WHERE fl_sesion=$fl_sesion");
    $cl_sesion = str_texto($row[0]);
    $fg_inscrito = $row[1];
  ...
  # Si no trae algunos datos los obtendra  cuando se incribio
  if(empty($registros)){
    $Queryextra = "SELECT a.fl_programa, b.nb_programa,''no_grado, '(no assigment)' nb_grupo, c.ds_login, '' fl_term, ";
    $Queryextra .= "CONCAT(c.ds_nombres, ' ', c.ds_apaterno, ' ', c.ds_amaterno) ds_nombre, d.ds_frecuencia, e.nb_periodo, ".ConsultaFechaBD('e.fe_inicio', FMT_FECHA).", ";
    $Queryextra .= "(SELECT no_semanas FROM k_programa_costos l WHERE l.fl_programa=a.fl_programa) no_semanas, d.fg_opcion_pago ";
    $Queryextra .= "FROM  k_ses_app_frm_1 a, c_programa b, c_usuario c, k_app_contrato d, c_periodo e ";
    $Queryextra .= "WHERE a.fl_programa = b.fl_programa AND a.cl_sesion=c.cl_sesion  AND a.cl_sesion=d.cl_sesion ";
    $Queryextra .= "AND a.fl_periodo = e.fl_periodo AND a.cl_sesion='$cl_sesion' ";
    $rowextra = RecuperaValor($Queryextra);
    $fl_programa = $rowextra[0];
    $nb_programa = $rowextra[1];
    $no_grado = $rowextra[2];
    $nb_grupo = $rowextra[3];
    $ds_login = $rowextra[4];
    $fl_term = $rowextra[5];
    $ds_nombre = $rowextra[6];
    $ds_frecuencia = $rowextra[7];
    $nb_periodo = $rowextra[8];
    $fe_inicio = $rowextra[9];
    $no_semanas = $rowextra[10];
      $meses_duracion = $no_semanas/4; //meses de duracion del programa
    $fg_opcion_pago = $rowextra[11];
    if(empty($no_grado) OR empty($fl_alumno)){
      $rowextra1 = RecuperaValor("SELECT MAX(fl_term) FROM k_alumno_term WHERE fl_alumno=$fl_alumno");
      $fl_term = $rowextra1[1];
      $rowextra2 = RecuperaValor("SELECT no_grado FROM k_term WHERE fl_term=$fl_term");
      $no_grado  = $rowextra2[0];
    }
  }
  ...
  if(!empty($fg_inscrito)){
    ...
    $Query .= ", DATE_FORMAT(fe_refund,'%M %d, %Y'), mn_tax_paypal, ds_tax_provincia FROM k_alumno_pago WHERE fl_term_pago=$fl_term_pago AND fl_alumno=$fl_alumno";
  }
  else{
    ...
    $Query .= ", DATE_FORMAT(fe_refund,'%M %d, %Y'), mn_tax_paypal, ds_tax_provincia FROM k_ses_pago WHERE fl_term_pago=$fl_term_pago AND cl_sesion='$cl_sesion'";
  }
  ...
  if(!empty($fl_term_pago) AND !empty($no_pago)){
    ...
    //$pago_normal_x_mes = round($pago_c_interes/$no_meses_op,2);
  ...
  #Pra los ya tienen fecha de pago.(ya pagado)
  if(!empty($data1)){
      
      if($mn_discount>0){
          $descuento=$mn_discount;
  ...
  $Query .="mn_pagado,ds_comentario, ds_transaccion, cl_metodo_pago, mn_tax_paypal, ds_tax_provincia FROM c_sesion  WHERE fl_sesion='$fl_sesion' ";
  $row=RecuperaValor($Query);
  $cl_metodo_app = $row[1];
  ...
  if(($mn_tax_paypal_appfee<=0)&&($cl_pais==38)){
    ...
    <table style="width:540px;">
      <tr>
        <td width="52%" rowspan="3">
          <div style="font-size:28px;">'.ObtenEtiqueta(516).'<br />
        ...
        <td width="18%">&nbsp;</td>
        <td colspan="2" width="30%" style="font-size:45px; font-weight:bold; text-align: center">Sales Receipt</td>
      </tr>
      <tr style="text-align:center;">
        <td width="18%">&nbsp;</td>
        <td width="15%" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  font-size:28px;">Date</td>
        <td width="15%" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  font-size:28px;">Sale No.</td>
      </tr>
      <tr style="text-align:center;">
        <td width="18%">&nbsp;</td>
        <td width="15%" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid; font-size:28px; ">'.$Date.'</td>
        <td width="15%" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  font-size:28px;">'.$fl_alumno_pago.'</td>
      </tr>
    </table>
    <br /><br />
    <table border="0" style="width:540px;">
      <tr >
        <td width="52%"; style="font-size:30px;   border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid; " ><br />Sold to</td>
        <td width="3%"></td>
        <td width="15%" style="text-align:center; font-size:30px;  border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  ">Cheque No.</td>
        <td width="15%" style="text-align:center; font-size:30px;   border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  ">Payment Method</td>
        <td width="15%" style="text-align:center; font-size:30px;   border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;  ">Payment Number</td>
      </tr>
      <tr>
        <td rowspan="3" style="font-size:28px;  border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;">'.$ds_fname.' '.$ds_lname.' '.$ds_mname.'<br>'.$nb_programa.'<br />' .$ds_add_number.'  '.$ds_add_street.'<br />'.$ds_add_city.', '.$ds_add_state.'<br />'.$ds_add_country.', '.$ds_add_zip.'<br /> Group '.$grupo.'</td>
        <td></td>
        <td style="text-align:center; font-size:28px;  border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;">'.$ds_cheque.'</td>
        <td style="text-align:center; font-size:28px;  border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;">'.$cl_metodo_pago.'</td>
        <td style="text-align:center; font-size:28px;  border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; border-top:1px #cccccc solid;"> '.$Payment_Number.'</td>        
      </tr>
    </table><br /><br /><br />';
  $htmlcontent .='
    <br /> <br /> <br /><br />
    <table border="1" style="width:540px;">
      <tr style="font-weight:bold; background-color:#FFFFFF; font-size:30px;" >
    ...
    if(!empty($fl_term_pago) AND !empty($no_pago)){
      ...
      if($mn_late_fee > 0){
        $htmlcontent .='
          <tr style="font-size:30px; padding:10px;">
      ...
      /*  $htmlcontent .='
        <tr style="font-size:30px;">
          ...
          <td style="text-align:center; border-left:1px #cccccc solid; border-right:1px #cccccc solid;  #cccccc solid;">1</td>
          <td style="text-align:right; border-left:1px #cccccc solid; border-right:1px #cccccc solid;  #cccccc solid;">'.number_format($adicional, 2, '.',',').'</td>
          <td style="text-align:right; border-left:1px #cccccc solid; border-right:1px #cccccc solid;  #cccccc solid;">'.number_format($adicional, 2, '.',',').'</td>
        </tr>';
		*/
      }
      ...
      if(!empty($fg_tax_rate)){
		  ...
		  if(($mn_tax_paypal_pago<=0)&&($cl_pais==38)){
      ...
      if($cl_pais==38 AND !empty($fg_tax_rate)){
        ...
        <tr style="font-size:30px;">
          ...
          <td style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; ">'.$ds_tax_provincia.': </td>
          <td align="right" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; " >'.number_format($mn_tax_paypal_pago,2,'.',',').'</td>
        </tr>';
      }
      $htmlcontent .='
        <tr style="border:1px ; font-size:30px;">
          ...
          <td align="right" style="border-left:1px #cccccc solid; border-right:1px #cccccc solid; border-bottom:1px #cccccc solid; " >'.number_format($total,2,'.',',').'</td>
        </tr>';
    }
    else{

      $htmlcontent .='
        <tr style="border-left:1px #cccccc border-right:1px #cccccc solid; font-size:30px;" >
          <td width="70%"; style="border-left:1px #cccccc solid; border-right:1px #cccccc solid;"><br />'.ObtenEtiqueta(584).'<br /></td>
          <td width="10%"; style="text-align:center; border:1px #cccccc solid; ">1</td>
          <td width="10%"; style="text-align:right; border:1px #cccccc solid; ">'.number_format($mn_pagado_app,2,'.',',').'</td>
          <td width="10%"; style="text-align:right; border:1px #cccccc solid;">'.number_format($mn_pagado_app,2,'.',',').'</td>
        </tr>';
      ...
      if($cl_pais==38 AND (!empty($fg_tax_rate) || empty($fg_tax_rate))){
        ...
        <tr style="font-size:30px;">
          ...
          <td>Subtotal: <br /> '.$ds_tax_provinciaapp.':  <br /> Total: </td>
          <td align="right">'.number_format($subtotal,2,'.',',').'<br />'.number_format($mn_tax_paypal_appfee,2,'.',',').'<br />'.number_format($total,2,'.',',').'</td>
        </tr>';
      }
      else{
        $htmlcontent .='
