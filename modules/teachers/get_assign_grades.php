<?php
  ...
  $row = RecuperaValor("SELECT MAX(no_equivalencia), cl_calificacion FROM c_calificacion WHERE no_equivalencia <= $max_calificacion");
  $ds_max_calificacion = $row[1];
  
  # Dialogo para asignar calificacion
  echo "
  <form name='datos' id='datos' method='POST' action='assign_grades.php'>
