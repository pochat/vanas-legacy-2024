<?php        
 ...
 // require PATH_ADM_HOME . "/lib/adobeconnect/LicenciaAdobeService.class.php";
  ...
  $Query  = "SELECT a.fe_clase,fl_clase_global ";
  $Query .= "from k_clase_cg a ";
  $Query .= "where a.fl_clase_cg = $fl_clase_cg ";
  ...
  #Recuperamos si es zoom.
  $QueryZ="SELECT fg_zoom from c_clase_global where fl_clase_global=$fl_clase_global ";
  ...
  $live_session_cg = ObtenRegistroLiveSessionCG($fl_clase_cg);
  
  // Valida existencia de la sesion
  if ($live_session_cg["fl_live_session_cg"] == '') {
    ...
    $welcome = ObtenTituloLeccionPorClaseGlobal($fl_clase_cg);
  ...
  $live_session_cg = ObtenRegistroLiveSessionCG($fl_clase_cg);
  $fl_live_session_cg = $live_session_cg["fl_live_session_cg"];
  ...
  // Mensaje de bienvenida, usamos Lesson title
  $welcome = $live_session_cg["ds_mensaje_bienvenida"];      
 ...
 // $licenciaService = new LicenciaAdobeService();
  ...
  if($fl_perfil == PFL_ESTUDIANTE) {
    ...
    #Se obtiene la url de conexxion a Zoom.
    $QueryLs  = "SELECT fl_live_session_cg, cl_licencia, cl_meeting_id,zoom_url ";
    $QueryLs .= "FROM k_live_sesion_cg WHERE fl_clase_cg = $fl_clase_cg";   
...
//    $LicenciaActual = $licenciaService->getLicenciaByClave($cl_licencia_actual); 
  ...
  $Query  = "SELECT fl_usuario FROM k_live_session_asistencia_cg WHERE fl_live_session_cg = $fl_live_session_cg AND fl_usuario = $fl_usuario ";
  $row = RecuperaValor($Query);
  
  if ($row[0] == '')  {
    $Query  = "INSERT INTO k_live_session_asistencia_cg ";
    $Query .= "(fl_live_session_cg, fl_usuario, cl_estatus_asistencia_cg, fe_asistencia_cg)";
  ...
  $urlJoinMeeting = $urlAdobe; 
   ...
   #2020 No borarar esto te redirige a adobeconect
  if($fg_zoom==1){
?>
    <script>
