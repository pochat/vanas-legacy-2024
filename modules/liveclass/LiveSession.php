<?php        
  ...
  # Recibe parametros
  $fl_clase = RecibeParametroNumerico('folio', True);
  
  if ($fl_clase == '' || $fl_clase == 0)
    exit;
  ...
  $Query  = "SELECT a.fe_clase,fl_grupo ";
  $Query .= "from k_clase a ";
  $Query .= "where a.fl_clase = $fl_clase ";
  ...
  $live_session = ObtenRegistroLiveSession($fl_clase);
    
  // Valida existencia de la sesion
  if ($live_session["fl_live_session"] == '') {
    ...
    $welcome = ObtenTituloLeccionPorClase($fl_clase);
  ...
  $live_session = ObtenRegistroLiveSession($fl_clase);
  $fl_live_session = $live_session["fl_live_session"];
  $meetingID = $live_session["ds_meeting_id"];        
  $attendeePW = $live_session["ds_password_asistente"];
  $moderatorPW = $live_session["ds_password_admin"];
  // Mensaje de bienvenida, usamos Lesson title
  $welcome = $live_session["ds_mensaje_bienvenida"];      
  ...
  if($fl_perfil == PFL_ESTUDIANTE) {
    ...
    #Se obtiene la url de conexion del estudiante.
    $QueryLs  = "select fl_live_session, cl_licencia, cl_meeting_id,zoom_url ";
    $QueryLs .= "from k_live_session where fl_clase = $fl_clase";   
  ...
  $Query  = "select fl_usuario from k_live_session_asistencia where fl_live_session = $fl_live_session and fl_usuario = $fl_usuario ";
  $row = RecuperaValor($Query);
  
  if ($row[0] == '')  {
    $Query  = "INSERT INTO k_live_session_asistencia ";
    $Query .= "(fl_live_session, fl_usuario, cl_estatus_asistencia, fe_asistencia)";
    $Query .= "VALUES ($fl_live_session, $fl_usuario, $estatus_asistencia, CURRENT_TIMESTAMP ) ";
...
<?php
      ...
      #No borrar esto te redirige al adobe conect.
