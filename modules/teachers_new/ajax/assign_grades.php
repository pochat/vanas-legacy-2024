<?php
  ...
  if(!empty($promediopgpa))
  EjecutaQuery("UPDATE c_alumno SET no_promedio_t='".round($promedio_x_t/$promediopgpa)."' WHERE fl_alumno=$fl_alumno");

#2023 backend
# Redirige al listado
echo"<a href='javascript:history.back()'  id='regresar'>Back and  Refresh the browser to see the assigned grade.</a>  ";
exit;
...
$ds_email_template = $ds_template_html->saveHTML();
	...
	$ds_email_alterative=RecoveryEmailAlternative($fl_post_usuario);
	$ds_email_responsable=RecoveryEmailResponsableStudent($fl_post_usuario);


	#Se envia copia de emmail si el studen asi lo elige.
	#if($ds_email_alterative)
	#SendNoticeMail($client, $from,''.$ds_email_alterative.'','', 'New Comment on VANAS Board', $ds_template_html);


  #SendNoticeMail($client, $from, $ds_email, $ds_email_responsable,''.$ds_email_responsable.'', 'New Grade Assigned', $ds_template_html);
  ...
  if(ObtenConfiguracion(81)){
	...
	#Se envia copia a
	if($ds_email_alterative)
		$apply.=";".$ds_email_alterative;
	if($ds_email_responsable)
		$apply.=";".$ds_email_responsable;
    ...
    if(empty($aprovado) && $fl_calificacion>0){
      ...
      $messageW .= "--".$separator."--".$eol;
        ...
        Mailer($ds_email,$template_warning,$messageW,'',$fileattweek,$fileName);
    ...
    if(empty($aprovadoPGPA)){
      ...
      $messagePGPA .= "--".$separator."--".$eol;
        ...
        Mailer($ds_email,$template_warningPGPA,$messagePGPA,'',$fileattterm,$fileNameterm);
    ...
    if($semana_actual==$semana_penultima AND empty($aprovadoTGPA)){
      ...
      $messagePGPA .= "--".$separator."--".$eol;
        ...
        Mailer($ds_email,$template_warningTGPA,$messagePGPA,'',$fileattprogram,$fileNamep);
