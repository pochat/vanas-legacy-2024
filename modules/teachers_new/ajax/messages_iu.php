<?php
  ...
  if(!empty($fl_usuario_dest) AND !empty($ds_mensaje)) {
    ...
    # Si destino es uno indica que es de FAME
    $destino =0;
    if(!empty($p))
      $destino=1;
     ...
     $Query="SELECT ds_ruta_avatar FROM c_maestro WHERE fl_maestro=$fl_usuario_ori ";
     $row=RecuperaValor($Query);
     $ds_imagen=str_uso_normal($row[0]); 
     
     if($ds_imagen)
         $img_avatar_ori="".PATH_MAE_IMAGES."/avatars/".$ds_imagen;
	 else
         $img_avatar_ori=ObtenAvatarUsuario($fl_usuario_ori);
