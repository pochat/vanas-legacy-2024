<?php
  ...
  if(!empty($ds_comment)) {
    ...
    if($fl_usuario != $fl_post_usuario){
		...
		#Se enia copia si estan marcados.
      ...
      SendNoticeMail($client, $from, $ds_post_email, ''.$ds_email_responsable.'', 'New Comment on VANAS Board', $ds_template_html);
    }
    ...
    if($tot_comments > 0){
      while($row=RecuperaRegistro($rs)){
        ...
        SendNoticeMail($client, $from, $ds_comm_email, ''.$ds_email_responsable.'', 'New Comment on VANAS Board', $ds_template_html);
      }
    }
  }
