<?php 
  ...
  function GetDesktopTabs($fl_alumno, $weeks_setting, $fl_maestro){
  	...
  	$result["weeks"] = $weeks;
    # Verificamos que alumno sea nuevo
    ...
    for($im=0;$row=RecuperaRegistro($rs5); $im++){
        ...
        $Querg="SELECT COUNT(*) FROM k_clase_grupo WHERE fl_grupo=$fl_grupo AND fl_maestro=$fl_maestro  ";
        $ros=RecuperaValor($Querg);
        if(!empty($ros[0])){
            $teacher_std=$fl_maestro;
    ...
    if(!empty($std_new) && $teacher_std==$fl_maestro){
      $result["tabs"] = array(
        "1" => "Video Lecture",
        "2" => "Video Brief",
        "3" => "Assignment",
  ...
  # se activan todas las semanas para el profesor
  ...
  # true = open tab, false = disable tab
  # se da la prioridad a los profesores para que puedan ver todos los tabs
  function GetDisabledTabs($weeks_setting){
		$semana_act = $weeks_setting["current"];
  	$max_semana = $weeks_setting["max"];
  	$weeks_advance = $weeks_setting["advance"];
          ...
          <div class="col-sm-5 padding-5" style="position:absolute;">
            <div class="carousel-inner profile-pic" style="background-color:rgba(255, 255, 255, 0.82);">
              <div id="user-profile-container" class="profile-container no-margin">
                <a class="pull-right" href="<?php echo PATH_N_MAE; ?>/index.php#ajax/messages.php?usr=<?php echo $fl_alumno; ?>">
                  <img src="<?php echo SP_IMAGES."/".ObtenNombreImagen(217); ?>" title="Send message" height="16" width="16" style="margin:4px;">
                </a>
              </div>                  
            </div>
          </div>
...
<script type="text/javascript">
	// Variables
	var student, disabledWeeks, disabledTabs, supervisor;
	student = <?php echo json_encode($fl_alumno); ?>;
	disabledWeeks = <?php echo json_encode(GetDisabledWeeks($weeks_setting)); ?>;
	disabledTabs = <?php echo json_encode(GetDisabledTabs($weeks_setting)); ?>;
	supervisor = <?php echo json_encode($fg_supervisor); ?>;

	// Desktop tab names
	var desktopTabs, defaultTabs;
	desktopTabs = <?php echo json_encode(GetDesktopTabs($fl_alumno, $weeks_setting, $fl_usuario)); ?>;
  ...
  $("#header_student").append(user.profile.ds_ruta_foto);
	...
	$(document).ready(function(){
		...
		desktopContainer.on("click", "ul > li:not(.disabled)", function(){
			...
			if(type === "week"){
				if(disabledTabs[index] === false && !supervisor){
