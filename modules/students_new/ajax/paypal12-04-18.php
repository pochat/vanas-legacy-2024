<?php
  ...
  $from = $from;
  $to = 'gflores2702@gmail.com';
  // EnviaMailHTML($from, $from, $to, "Pago de paypal", "Esta a punto de realizarce un pago");
  if(DEBUG_LOG == 1)
  ...
  // EnviaMailHTML($from, $from, $to, "Pago de paypal", "Van en el pasode curl_init");
  if(DEBUG_LOG == 1)
  ...
  // EnviaMailHTML($from, $from, $to, "Pago de paypal", "Llego a curl_exec");
  if(DEBUG_LOG == 1)
  ...
  if (strcmp ($res, "VERIFIED") == 0) {
    ...
    if($payment_status == "Completed" AND !$existe_transaccion AND $valida_email AND !$existe_term_pago){
      ...
      if(strpos($custom, "PC|") !== False OR !$type_payment)
       ...
       // EnviaMailHTML($from, $from, $to, "Pago de palpal", "Se realizo el pago de paypal");
       if(DEBUG_LOG == 1)
  ...
  function AppPro($fl_alumno,$payment_status,$mn_app_fee,$mn_x_due,$ds_transaccion, $fg_total_programa, $fg_tax_rate=0, $fl_template=0, $mn_tax_paypal=0, $payment_date){
    ...
    $QueryT .= "a.ds_add_country,a.ds_add_state, c.mn_app_fee, c.mn_tot_tuition, b.fl_term, b.fl_term_ini ";
