<?php
  ...
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "***************************************INICIAMOS CON EL PAGO*******************************************************************************".$ch. PHP_EOL, 3, LOG_FILE);
  // Read POST data
  ...
  foreach ($myPost as $key => $value) {
    ...
    $req .= "&$key=$value";
    if(DEBUG_LOG == 1)
      error_log(date('[Y-m-d H:i e] '). "REG GABO:".$req. PHP_EOL, 3, LOG_FILE);
  }
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "function_exists:".$get_magic_quotes_exists. PHP_EOL, 3, LOG_FILE);
  ...
  if ($ch == FALSE) {
    if(DEBUG_LOG == 1)
		error_log(date('[Y-m-d H:i e] '). "ERROR CH:".$ch. PHP_EOL, 3, LOG_FILE);  
  
    return FALSE;
  }
  curl_setopt($ch, CURLOPT_HTTP_VERSION, CURL_HTTP_VERSION_1_1);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_HTTP_VERSION ".CURLOPT_HTTP_VERSION." <> CURL_HTTP_VERSION_1_1".CURL_HTTP_VERSION_1_1. PHP_EOL, 3, LOG_FILE);
  curl_setopt($ch, CURLOPT_POST, 1);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_POST".CURLOPT_POST. PHP_EOL, 3, LOG_FILE);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER,1);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_RETURNTRANSFER".CURLOPT_RETURNTRANSFER. PHP_EOL, 3, LOG_FILE);  
  curl_setopt($ch, CURLOPT_POSTFIELDS, $req);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_POSTFIELDS".CURLOPT_POSTFIELDS. PHP_EOL, 3, LOG_FILE);  
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "REQ:".$req. PHP_EOL, 3, LOG_FILE);
  ...
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_SSL_VERIFYPEER".CURLOPT_SSL_VERIFYPEER. PHP_EOL, 3, LOG_FILE);
  curl_setopt($ch, CURLOPT_SSL_VERIFYHOST, 2);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_SSL_VERIFYHOST".CURLOPT_SSL_VERIFYHOST. PHP_EOL, 3, LOG_FILE);
  curl_setopt($ch, CURLOPT_FORBID_REUSE, 1);
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_FORBID_REUSE".CURLOPT_FORBID_REUSE. PHP_EOL, 3, LOG_FILE);  
  if(DEBUG == true) {
    curl_setopt($ch, CURLOPT_HEADER, 1);
    if(DEBUG_LOG == 1)
      error_log(date('[Y-m-d H:i e] '). "CURLOPT_HEADER".CURLOPT_HEADER. PHP_EOL, 3, LOG_FILE);
    curl_setopt($ch, CURLINFO_HEADER_OUT, 1);
    if(DEBUG_LOG == 1)
      error_log(date('[Y-m-d H:i e] '). "CURLINFO_HEADER_OUT".CURLINFO_HEADER_OUT. PHP_EOL, 3, LOG_FILE);
  }
  ...
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_CONNECTTIMEOUT".CURLOPT_CONNECTTIMEOUT. PHP_EOL, 3, LOG_FILE);
  curl_setopt($ch, CURLOPT_HTTPHEADER, array('Connection: Close'));
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "CURLOPT_HTTPHEADER".CURLOPT_HTTPHEADER. PHP_EOL, 3, LOG_FILE);

  // CONFIG: Please download 'cacert.pem' from "http://curl.haxx.se/docs/caextract.html" and set the directory path
  // of the certificate as shown below. Ensure the file is readable by the webserver.
  // This is mandatory for some environments.
  $cert = "/var/www/html/vanas/modules/students_new/ajax/paypal_cacert/cacert.pem";
  curl_setopt($ch, CURLOPT_CAINFO, $cert);
  if ( !($res = curl_exec($ch)) ) {
    // error_log("Got " . curl_error($ch) . " when processing IPN data");
    if(DEBUG_LOG == 1){
      error_log(date('[Y-m-d H:i e] '). "when processing IPN data :".curl_error($ch). PHP_EOL, 3, LOG_FILE);
    }
    curl_close($ch);
    exit;
  }
  ...
  if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "curl_exec". PHP_EOL, 3, LOG_FILE);
  ...
  if (strcmp ($res, "VERIFIED") == 0) {
    ...
    if(strpos($custom, "|T") !== False){
      if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). "if(strpos($custom, '|T') !== False){ ". PHP_EOL, 3, LOG_FILE);
      ...
      if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). "Verificamos si es un pago completo". PHP_EOL, 3, LOG_FILE);
    ...
    if($payment_status == "Completed" AND !$existe_transaccion AND $valida_email AND !$existe_term_pago){
      if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). " Se van a realizar el pago". PHP_EOL, 3, LOG_FILE);
      ...
      if(strpos($custom, "PC|") !== False OR !$type_payment){
		...
		#Se recuepra de sus datos
		if(empty($mn_app_fee)){
			 
			 $Query="SELECT mn_app_fee FROM k_app_contrato WHERE cl_sesion='$fl_alumno' ";
		     $row=RecuperaValor($Query);
			 $mn_app_fee=!empty($row[0])?$row[0]:$mn_total;
			 error_log(date('[Y-m-d H:i e] '). "1. No venia e app fee entonces recuepra $Query = $mn_app_fee". PHP_EOL, 3, LOG_FILE);
       ...
       if(DEBUG_LOG == 1)
    error_log(date('[Y-m-d H:i e] '). "***************************************FINALIZAMOS CON EL PAGO*******************************************************************************".$ch. PHP_EOL, 3, LOG_FILE);
    }    
  }else{
  ...
  function AppPro($fl_alumno,$payment_status,$mn_app_fee,$mn_x_due,$ds_transaccion, $fg_total_programa, $fg_tax_rate=0, $fl_template=0, $mn_tax_paypal=0, $payment_date){
    ...
    if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). " Entro al AppPro ". PHP_EOL, 3, LOG_FILE);
    ...
    $QueryT = "SELECT b.fl_term_ini, ";
    $QueryT .= "a.ds_add_country,a.ds_add_state, c.mn_app_fee, c.mn_tot_tuition, b.fl_term, b.fl_term_inii,a.fl_programa ";
    ...
    $fl_term_pago = $row[0];
    if($fl_term_pago==0){
      $roww = RecuperaValor("SELECT g.fl_term_pago FROM k_term_pago g WHERE g.fl_term=".$row[5]);
      $fl_term_pago = $roww[0];
    }
    else{
      $roww = RecuperaValor("SELECT g.fl_term_pago FROM k_term_pago g WHERE g.fl_term=".$row[6]);
      $fl_term_pago = $roww[0];
    }
    ...
    $tax_tuition = 0;
    $fl_programa = $row[7];
    # Verificamos si registraron algun cupon
	# Si lo hay actualiza su contrato con el descuento
	$Queri = "SELECT ds_code, ds_descuento FROM k_app_cupon WHERE fl_sesion=(SELECT fl_sesion FROM c_sesion WHERE cl_sesion='".$fl_alumno."')";
	if(DEBUG_LOG==1)
		error_log(date('[Y-m-d H:i e] '). " TVerifica descuento".$Queri. PHP_EOL, 3, LOG_FILE);
	$roi = RecuperaValor($Queri);		
	if(!empty($roi[0])){
		if(DEBUG_LOG==1)
			error_log(date('[Y-m-d H:i e] '). " Tiene un descuento".$Queri. PHP_EOL, 3, LOG_FILE);
		...
		# Calculos
		$mn_discount = $roi[1];
		$ds_discount = ObtenEtiqueta(2298).$roi[0];
		...
		$r .= "mn_c_due=".$mn_c_due.", mn_c_paid=".$mn_c_paid.", mn_d_due=".$mn_d_due.", mn_d_paid=".$mn_d_paid." WHERE cl_sesion='".$fl_alumno."'";
		EjecutaQuery($r);
		if(DEBUG_LOG==1)
			error_log(date('[Y-m-d H:i e] '). " Actualiza datos para descuento".$r. PHP_EOL, 3, LOG_FILE);
		# Eliminamos el cupon
		EjecutaQuery("DELETE FROM k_app_cupon WHERE fl_sesion=(SELECT fl_sesion FROM c_sesion WHERE cl_sesion='".$fl_alumno."')");
		if(DEBUG_LOG==1)
			error_log(date('[Y-m-d H:i e] '). " Elimina el descuento temporal". PHP_EOL, 3, LOG_FILE);
	}
    ...
    # Cambiaron las condiciones siempre se cobrar tax a canadienses ya sea programa corto o largo pero siempre debe tener activado el fg_tax_rate 
    //if($mn_tax_paypal>0 AND !empty($fg_tax_rate) AND $ds_add_country==38){ 2017-02-08
    if($mn_tax_paypal>0 && $ds_add_country==38){
      if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). " Usuario de CANADA". PHP_EOL, 3, LOG_FILE);
      ...
      if(DEBUG_LOG==1)
        error_log(date('[Y-m-d H:i e] '). " Usuario de DE OTRO PAIS". PHP_EOL, 3, LOG_FILE);
    }
	...
	if(empty($mn_app_fee)){
		 ...
		 $Query="SELECT mn_app_fee FROM k_app_contrato WHERE cl_sesion='$fl_alumno' ";
		 $row=RecuperaValor($Query);
		 $mn_app_fee=!empty($row[0])?$row[0]:0;
		 error_log(date('[Y-m-d H:i e] '). "2. No venia e app fee entonces recuepra $Query = $mn_app_fee". PHP_EOL, 3, LOG_FILE);
