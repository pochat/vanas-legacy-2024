<?php
	...
	//$dom->loadHTML($ds_template);
	@$dom->loadHTML($ds_template);
	$xpath = new DOMXPath($dom);

	//$table = $dom->getElementById('board-container');
	$tds = $xpath->query('//table[@id="board-container"]//td');
	...
	$Query .= "WHERE b.fg_activo='1' AND fl_perfil IS NOT NULL AND fl_perfil<>0 LIMIT 1 ";
	...
	for($i=0; $student=RecuperaRegistro($students); $i++){
		...
		EnviaMailHTML('', $from, $ds_email, "Weekly Vanas Board Digest", $ds_template_email);
	}
?>
