...
class TCPDF {
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		switch($dest) {
			...
			case 'E': {
                                // MDB 27/may/2015
                                // Este codigo tenia definido como fin de linea "\r\n" lo que hacia que generara un archivo que se brincaba 2 lineas
                                // y que hacia que no se pudiera abrir, la codificacion era incorrecta.
                                // Eliminando el \r se genera correctamente, por cuestiones de compatibilidad entre sistemas operativos 
                                // agregue la linea usando PHP_EOL pero funciona bien con "\n".

                                $endOfLine = PHP_EOL; //"\n";
				// Return PDF as base64 mime multi-part email attachment (RFC 2045)
				$retval = 'Content-Type: application/pdf;'. $endOfLine;
				$retval .= ' name="'.$name.'"'. $endOfLine;
				$retval .= 'Content-Transfer-Encoding: base64'. $endOfLine;
				$retval .= 'Content-Disposition: attachment;'. $endOfLine;
				$retval .= ' filename="'.$name.'"'. $endOfLine .  $endOfLine;
				$retval .= chunk_split(base64_encode($this->getBuffer()), 76,  $endOfLine);
				return $retval;
			}
			case 'S': {
