<?php
  ...
  /*$Query .= ",(SELECT cl_calificacion FROM c_calificacion k, c_alumno l WHERE no_min <= h.no_promedio_t AND no_max >= l.no_promedio_t AND l.fl_alumno=a.fl_usuario) '".ObtenEtiqueta(431)."' ";*/
  $Query .= ", cl_calificacion ";
  $Query .= "FROM c_usuario a, c_perfil b, c_sesion c, k_ses_app_frm_1 d, c_programa e, k_pctia f, c_periodo g,  c_pais i, c_alumno h ";
  $Query .= "LEFT JOIN c_calificacion k ON(h.no_promedio_t>0 AND h.no_promedio_t BETWEEN no_min AND  no_max ) ";
  ...
  $Query .= "AND a.fl_usuario = f.fl_alumno AND d.fl_periodo=g.fl_periodo AND a.fl_usuario=h.fl_alumno AND a.fl_perfil=3 AND f.fe_completado < CURDATE() AND a.fl_usuario=h.fl_alumno ";
