<?php
  ...
  for($i=0; $row2=RecuperaRegistro($rs2); $i++){
    ...
    $countries_basic[$i] = (Object) array(
      "label" => $ds_pais,
      "count" => $count
    );
  }
  ...
  $Query2  = "SELECT nb_programa, ds_pais, COUNT(1) ";
  $Query2 .= "FROM c_usuario a ";
  $Query2 .= "LEFT JOIN k_ses_app_frm_1 b ON b.cl_sesion=a.cl_sesion ";
  $Query2 .= "LEFT JOIN c_programa c ON c.fl_programa=b.fl_programa ";
  $Query2 .= "LEFT JOIN c_pais d ON d.fl_pais=b.ds_add_country ";
  $Query2 .= "WHERE fl_usuario IN ($students) ";
  $Query2 .= "GROUP BY nb_programa, ds_pais ";
  $rs2 = EjecutaQuery($Query2);
  for($i=0; $row2=RecuperaRegistro($rs2); $i++){
    $nb_programa = $row2[0];
    $ds_pais = $row2[1];
    ...
    $countries_programs[$i] = (Object) array(
      "program" => $nb_programa,
      "label" => $ds_pais,
      "count" => $count
    );
  ...
  <script type='text/javascript'>
    (function($){
      // General configs
      var basic = ".json_encode($countries_basic).";
      var programs = ".json_encode($countries_programs).";
      ...
      // Legends
      var legendPanel = $('#legend_4_panel');
      var legendRectSize = 18;
      var legendSpacing = 4;

      // Pie chart - basic
      var svg = d3.select('#panel_4')
        .append('svg')
        .attr('width', width)
        .attr('height', height)
        .attr('class', 'basic')
      ...
      $('#panel_4 > form > input').click(function(){
        var input = $(this);
        var svgBasic = $('#panel_4 > svg.basic');
        var svgProgram = $('#panel_4 > svg.program');
