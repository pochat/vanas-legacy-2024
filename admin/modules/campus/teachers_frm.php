<?php
  ...
  Forma_CampoTexto(ObtenEtiqueta(120).' '.ETQ_FMT_FECHA, True, 'fe_nacimiento', $fe_nacimiento, 10, 10, $fe_nacimiento_err);
    ...
    for($i=0;$row= RecuperaRegistro($rs);$i++){
      ...
      <tr>
        ...
        if($fg_error AND !empty($mn_lecture_fee_err[$i])){
          $class='css_input_error';
        }
        else{
          $class = 'css_input';
        }
          CampoTexto('mn_lecture_fee_'.$i, $mn_lecture_fee, 10, 10, $class, False, 'style="text-align:right"');
        echo "
        </td>
        <td>";
        if($fg_error AND !empty($mn_extra_fee_err[$i]))
          ...
          $class = "css_input";
          CampoTexto('mn_extra_fee_'.$i, $mn_extra_fee, 10, 10, $class, False, 'style="text-align:right" ');
        echo "
        </td>
      </tr>";
