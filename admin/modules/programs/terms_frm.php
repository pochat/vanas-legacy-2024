<?php
  ...
  if(!empty($clave)) {
    ...
    for($i = 0; $i < $tot_semanas; $i++) {  
      ...
      CampoTexto('hr_publicacion_'.$i, $hr_publicacion[$i], 5, 3, '', False);
      echo "</td>
        <td align='left'>";
      if($fe_entrega_err[$i])
        $ds_clase = 'css_input_error';
        ...
        for($j = 1; $j <= $no_payments[$i]; $j++) {
          if($j == 1){
            ...
            echo "
              <tr align='center'>
                <td colspan='2' >
                  <input type='button' value='Date Calculate $opciones[$i]' align='center' OnClick='calculafechas($fe_inicioD, $fe_inicioM, $fe_inicioA,$no_payments[$i], $fl_programa, $i, $fl_term_ini );'>
                </td>
              </tr>";
          }
          Forma_CampoTexto("Payment $j ".ETQ_FMT_FECHA, True, 'fe_pago_'.$i.'_'.$j, $fe_pago[$i][$j], 10, 10, $fe_pago_err[$i][$j]);
