<?php
  ...
  if(!empty($fe_pago) AND !ValidaFecha($fe_pago))
    $fe_pago_err = ERR_FORMATO_FECHA;
  
  
	# Regresa a la forma con error
  $fg_error = $nb_periodo_err || $fe_inicio_err || $fe_pago_err;
  ...
  if($fg_error) {
    ...
    Forma_CampoOculto('fe_pago' , $fe_pago);
    Forma_CampoOculto('fe_pago_err' , $fe_pago_err);
...
<script>
  ...
  if(empty($clave)) {
    $Query  = "INSERT INTO c_periodo (nb_periodo, fe_inicio, fe_pago, fg_activo) ";
    $Query .= "VALUES('$nb_periodo', $fe_inicio, $fe_pago, '$fg_activo') ";
  }
  else {
    $Query  = "UPDATE c_periodo SET nb_periodo='$nb_periodo', fe_inicio=$fe_inicio, fe_pago=$fe_pago, fg_activo='$fg_activo' ";
