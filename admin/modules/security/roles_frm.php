<?php
  ...
  if(!empty($clave)) {
    ...
    Forma_MuestraTabla($Query, TB_LN_NNN, 'usuarios', '', '80%');
    Forma_Espacio( );
  }
  ...
  while($row = RecuperaRegistro($Rows)) {
    ...
    echo "
  <tr>
    <td align=right valign=top class='css_prompt'>$ds_modulo:</td>
    <td>
      <table border='".D_BORDES."' cellpadding='3' cellspacing='0' class='css_default'>\n";
    ...
    while($row2 = RecuperaRegistro($Rows2)) {
      ...
      $total_permisos++;
      printf("
        <tr><td colspan=5><br>$ds_funcion<input type=hidden name=F$total_permisos value='$fl_funcion'></td></tr>
        <tr>");
      if($fg_solo_ejecucion == "0") { 
        printf("
          <td class='css_etq_texto'><input type=checkbox name=X$total_permisos");
        if(($fl_funcion == $fl_funcion_fun)&&($fg_ejecucion == 1))
          printf(" checked");
        printf(">%s</td>
          <td class='css_etq_texto'><input type=checkbox name=D$total_permisos", ObtenEtiqueta(101)); // Ver listado
        if(($fl_funcion == $fl_funcion_fun)&&($fg_detalle == 1))
          printf(" checked");
        printf(">%s</td>
          <td class='css_etq_texto'><input type=checkbox name=C$total_permisos", ObtenEtiqueta(102)); // Ver detalle
        if(($fl_funcion == $fl_funcion_fun)&&($fg_modificacion == 1))
          printf(" checked");
        printf(">%s</td>
          <td class='css_etq_texto'><input type=checkbox name=A$total_permisos", ETQ_EDITAR);
        if(($fl_funcion == $fl_funcion_fun)&&($fg_alta == 1))
          printf(" checked");
        printf(">%s</td>
          <td class='css_etq_texto'><input type=checkbox name=B$total_permisos", ETQ_INSERTAR);
        if(($fl_funcion == $fl_funcion_fun)&&($fg_baja == 1))
          printf(" checked");
        printf(">%s</td>", ETQ_ELIMINAR);
      }
      else {
        printf("
          <td colspan=5 class='css_etq_texto' align=left><input type=checkbox name=X$total_permisos");
        if(($fl_funcion == $fl_funcion_fun)&&($fg_ejecucion == 1))
          printf(" checked");
        printf(">%s
          ...
          </td>", ObtenEtiqueta(103)); // Ejecutar
      }
      echo "
        </tr>\n";
    }
    if($total_permisos == 0)
      printf("<tr><td class='css_etq_texto'>%s</td></tr>", ObtenEtiqueta(104)); // No existen funciones para este m&oacute;dulo.
    echo "
      </table>
    </td>
  </tr>\n";
