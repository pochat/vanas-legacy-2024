<?php
  ...
  $Query  = "SELECT fl_contenido, lb_funcion '".ObtenEtiqueta(154)."', ds_funcion '".ETQ_DESCRIPCION."', ds_estado '".ObtenEtiqueta(200)."', ";
  $Query .= "ds_activo '".ObtenEtiqueta(199)."|center', ds_ini '".ObtenEtiqueta(190)."', ds_fijo '".ObtenEtiqueta(161)."|center' ";
  $Query .= "FROM ( ";
  $Query .= "SELECT a.fl_contenido, nb_funcion, tr_funcion, ds_funcion, a.no_orden no_orden, ";
  ...
  $Query .= "AND b.cl_tipo_contenido=".TC_LIGA." ";
  $Query .= "ORDER BY b.fl_modulo, b.no_orden, fe_evento DESC) AS principal ";
  if(!empty($criterio)) {
    switch($actual) {
      case 1: $Query .= "WHERE nb_funcion LIKE '%$criterio%' OR tr_funcion LIKE '%$criterio%' "; break;
      case 2: $Query .= "WHERE ds_funcion LIKE '%$criterio%' "; break;
      case 3: $Query .= "WHERE ds_estado LIKE '%$criterio%' "; break;
      default:
        $Query .= "WHERE nb_funcion LIKE '%$criterio%' OR tr_funcion LIKE '%$criterio%' ";
        $Query .= "OR ds_funcion LIKE '%$criterio%' OR ds_estado LIKE '%$criterio%' ";
    }
  }
  
  # Muestra pagina de listado
  $opc = array(ObtenEtiqueta(154), ETQ_DESCRIPCION, ObtenEtiqueta(200));
  PresentaPaginaListado(FUNC_LIGAS, $Query, TB_LN_IUD, True, False, $opc);
