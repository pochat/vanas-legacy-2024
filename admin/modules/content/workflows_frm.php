<?php
  ...
  <tr class='$clase'>
    ...
    if($ds_nivel_err[$i])
      ...
      $ds_clase = 'css_input';
    CampoTexto("ds_nivel_$i", $ds_nivel[$i], 50, 25, $ds_clase);
    echo "</td>\n";
    if(FG_TRADUCCION) {
      echo "<td>";
      CampoTexto("tr_nivel_$i", $tr_nivel[$i], 50, 25, 'css_input');
      echo "</td>\n";
    }
    else
      Forma_CampoOculto("tr_nivel_$i", $tr_nivel[$i]);
    echo "<td>";
    CampoLOV("fl_perfil_$i", $fl_perfil[$i], "nb_perfil_$i", $nb_perfil[$i], 20, LOV_PERFILES, ETQ_SELECCIONAR.' '.ObtenEtiqueta(110), LOV_TIPO_CHKBOX, LOV_MEDIANO, '', 'css_input');
    echo "</td>
    <td>";
    CampoLOV("fl_usuario_$i", $fl_usuario[$i], "nb_usuario_$i", $nb_usuario[$i], 20, LOV_USUARIOS, ETQ_SELECCIONAR.' '.ETQ_USUARIO, LOV_TIPO_CHKBOX, LOV_GRANDE, "fl_perfil_$i", 'css_input');
    echo "</td>
  </tr>\n";
  }
  $fg_error = False;
