<?php
  ...
  $interes_a = RecibeParametroHTML('interes_a');
  ...
  $interes_b = RecibeParametroHTML('interes_b');
  ...
  $interes_c = RecibeParametroHTML('interes_c');
  ...
  $interes_d = RecibeParametroHTML('interes_d');
  $fg_fulltime = RecibeParametroNumerico('fg_fulltime');
  $fg_taxes = RecibeParametroNumerico('fg_taxes');
  $fg_total_programa = RecibeParametroNumerico('fg_total_programa');
  ...
  $fg_error = $nb_programa_err || $ds_duracion_err || $ds_tipo_err || $no_orden_err || $no_grados_err || $fl_template_err || 
              $no_horas_err || $no_semanas_err || $ds_credential_err;
...
<script>
  ...
  if(empty($clave)) {
    $Query  = "INSERT INTO c_programa (nb_programa, ds_duracion, ds_tipo, no_orden, no_grados, fl_template, fg_fulltime, fg_taxes, fg_total_programa) ";
    $Query .= "VALUES('$nb_programa', '$ds_duracion', '$ds_tipo', $no_orden, $no_grados, $fl_template, '$fg_fulltime', '$fg_taxes', '$fg_total_programa') ";
    EjecutaQuery($Query);
    ...
    $Query .= "no_horas, no_semanas, ds_credential, cl_delivery, ds_language, cl_type) ";
    ...
    $Query .= "no_orden=$no_orden, no_grados=$no_grados, fl_template=$fl_template, fg_fulltime='$fg_fulltime', fg_taxes='$fg_taxes', fg_total_programa='$fg_total_programa' ";
