<?php
  ...
  $fg_error = $cl_etiqueta_err || $nb_etiqueta_err || $ds_etiqueta_err;
  if($fg_error) {
...
<script>
  ...
  if(!empty($cl_etiqueta)) {
    ...
    $Query .= "SET nb_etiqueta='$nb_etiqueta', ds_etiqueta='$ds_etiqueta', tr_etiqueta='$tr_etiqueta' ";
    $Query .= "WHERE cl_etiqueta = $cl_etiqueta";
  }
  else {
    $Query  = "INSERT INTO c_etiqueta (cl_etiqueta, nb_etiqueta, ds_etiqueta, tr_etiqueta) ";
    $Query .= "VALUES($cl_etiqueta_nueva, '$nb_etiqueta', '$ds_etiqueta', '$tr_etiqueta')";
	}
	EjecutaQuery($Query);
