  ...
  $ds_template_html = new simple_html_dom();
	...
	while($row=RecuperaRegistro($rs)){
		...
		if($tot_students > 0){
			...
			while($row2=RecuperaRegistro($rs2)){
		        ...
		        SendNoticeMail($client, $from, $ds_email_alterative, "", "Upcoming Live Session", $ds_template_html);
				...
				SendNoticeMail($client, $from, $ds_email, "".$ds_email_responsable."", "Upcoming Live Session", $ds_template_html);
			}
      ...
      $te_lname = $row3[2];
			...
			SendNoticeMail($client, $from, $ds_email, "", "Upcoming Live Session", $ds_template_html);
		}
	}
	...
	while($row=RecuperaRegistro($rs)){
        ...
        $ds_template_html->load($ds_email_template);
				...
				SendNoticeMail($client, $from, $ds_email, "", "Upcoming Extra Live Session", $ds_template_html);
			}
			...
			# Generate the email template with the variables
			echo $ds_email_template = GenerateTemplate($ds_te_extra_template, $variables);
