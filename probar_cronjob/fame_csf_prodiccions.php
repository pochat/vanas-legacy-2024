...
$Query="SELECT a.fl_instituto,a.ds_instituto,b.fl_usuario,b.ds_email,b.ds_nombres,b.ds_apaterno
		FROM c_instituto a
        JOIN c_usuario b ON b.fl_usuario=a.fl_usuario_sp 
		WHERE a.fl_instituto=185 ";
...
#Tipos de archvhivos para identifcar.
//$upload_type="student";
//$upload_type="teacher";
//$upload_type="school";
...
foreach($scan as $new_file){
    ...
    if($ext=="csv"){
        ...
        if(!empty($archivo_valido)){
            ...
            $Queryu ='INSERT INTO stage_uploads(user_id,upload_file_path,upload_file_name,upload_type,upload_date,status_cd,start_time,end_time,proc_status)';
            $Queryu.='VALUES('.$fl_usuario.', "'.$path_dest_local.'","'.$new_file.'","'.$upload_type.'", CURRENT_TIMESTAMP,"NEW",CURRENT_TIMESTAMP,CURRENT_TIMESTAMP,"")';
            ...
            if(($upload_type=='school')||($upload_type=='School')||($upload_type=='SCHOOL')){
                ...
                EjecutaQuery("UPDATE c_instituto SET fg_scf_revisado=0 WHERE fg_scf='1' ");
                
                GeneraLog($file_name_txt,date("F j, Y, g:i a")."UPDATE c_instituto SET fg_scf_revisado=0 WHERE fg_scf='1'");
                ...
                if ($file = fopen($ruta_completa_archivo, "r")){
                    ...
                    while ($data = fgetcsv ($file, 0, ",")){
                        ...
                        if($primeracarga==1){
                            ...
                            $Query='SELECT ds_instituto,school_id FROM c_instituto WHERE ds_instituto="'.$nb_school.'" AND school_id="'.$school_id.'" AND fg_scf="1" ';
                            $row=RecuperaValor($Query);
                            ...
                            if(!empty($row[0])){
                                ...
                                #Verifica si existe ese registro en FAME.()
                                $Query='SELECT ds_instituto,school_id FROM c_instituto WHERE school_id="'.$school_id.'" AND fg_scf="1" ';
                                $row=RecuperaValor($Query);
                                $ds_instituto_db=$row['ds_instituto'];
                                $school_id_db=$row['school_id'];
            ...
            if(($upload_type=='student')||($upload_type=='Student')||($upload_type=='STUDENT')){
                ...
                $Query="SELECT COUNT(*) FROM st_students  ";
                $row=RecuperaValor($Query);
                $conta_reg_ini=$row[0];
                            ...
                            if((!empty($first_name))&&(!empty($teacher_id))){
                                ...
                                if($primeracarga==1){
                                    ...
                                    $Query='SELECT ds_login,ds_alias,ds_nombres,ds_apaterno,ds_email,b.fg_scf,b.fl_usuario,c.course_id  
                                                        ...
                                                        WHERE d.school_id="'.$school_id.'" AND  b.ds_login="'.$username.'" AND ds_nombres="'.$first_name.'" AND ds_apaterno="'.$last_name_pat.'" AND ds_email="'.$email.'" AND b.fg_scf=1 AND fl_perfil_sp=15
                                                        ';
                                    ...
                                    if(!empty($row[0])){
                                        ...
                                        #Se inserta en primera instancia la bitacora de estudiantes.
                                        $Queryui ='INSERT INTO st_students (upload_id,student_id,operation_code,email,first_name,last_name_pat,last_name_mat,teacher_id,school_id,course_id,status,gpa,groups,school_level,fl_perfil_sp,username)';
                                        $Queryui.='VALUES('.$fl_upload.',"'.$student_id.'","NO_CHANGE","'.$email.'","'.$first_name.'","'.$last_name_pat.'","","'.$teacher_id.'","'.$school_id.'","'.$course_id.'","'.$status.'",'.$gpa.',"'.$grupo.'",'.$school_level.',"15","'.$username.'")';
                                        $st_id=EjecutaInsert($Queryui);
                                        ...
                                        EjecutaQuery('UPDATE c_usuario SET fg_scf_revisado=1,fg_activo="1" WHERE fl_usuario='.$fl_usuario_student.' AND fl_perfil_sp='.PFL_ESTUDIANTE_SELF.' AND fg_scf="1" ');
                                        ...
                                        $Query='SELECT ds_login,ds_alias,ds_nombres,ds_apaterno,ds_email,b.fg_scf,b.fl_usuario,c.course_id  
                                                        ...
                                                        JOIN c_usuario b ON b.fl_usuario=a.fl_alumno_sp  
                                                        JOIN k_usuario_programa c ON c.fl_usuario_sp=b.fl_usuario AND c.course_id="'.$course_id.'"
                                                        WHERE  b.ds_login="'.$username.'" AND b.fg_scf=1 AND fl_perfil_sp="'.PFL_ESTUDIANTE_SELF.'"  ';
                                        $row=RecuperaValor($Query);
                                        $ds_login_db=$row['ds_login'];
                                        $username_db=$row['ds_login'];
                                        $course_id_db=$row['course_id'];
                                        ...
                                        if(($username_db==$username)&&($course_id_db==$course_id)){
                                            ...
                                            if($username_db==$username){
                                                ...
                                                #Se inserta en primera instancia la bitacora de estudiantes.
                                                $Queryui ='INSERT INTO st_students (upload_id,student_id,operation_code,email,first_name,last_name_pat,last_name_mat,teacher_id,school_id,course_id,status,gpa,groups,school_level,fl_perfil_sp,username)';
                                                $Queryui.='VALUES('.$fl_upload.',"'.$student_id.'","MODIFY","'.$email.'","'.$first_name.'","'.$last_name_pat.'","","'.$teacher_id.'","'.$school_id.'","'.$course_id.'","'.$status.'",'.$gpa.',"'.$grupo.'",'.$school_level.',"15","'.$username.'")';
                                                $st_id=EjecutaInsert($Queryui);
                                                ...
                                                #Se inserta en primera instancia la bitacora de estudiantes.
                                                $Queryui ='INSERT INTO st_students (upload_id,student_id,operation_code,email,first_name,last_name_pat,last_name_mat,teacher_id,school_id,course_id,status,gpa,groups,school_level,fl_perfil_sp,username)';
                                                $Queryui.='VALUES('.$fl_upload.',"'.$student_id.'","ADD","'.$email.'","'.$first_name.'","'.$last_name_pat.'","","'.$teacher_id.'","'.$school_id.'","'.$course_id.'","'.$status.'",'.$gpa.',"'.$grupo.'",'.$school_level.',"15","'.$username.'")';
                                                $st_id=EjecutaInsert($Queryui);
                ...
                #Revisamos cuanto no fueron porcesados de los existentes en FAME Y determinamos que fuweron eliminados.
                $Query="SELECT COUNT(*)FROM c_usuario WHERE fg_scf_revisado=0 AND fg_scf='1' AND fg_activo='1' AND fl_perfil_sp=".PFL_ESTUDIANTE_SELF."  ";
                $ro=RecuperaValor($Query);
                $deleted_count=$ro[0];
                ...
                if($permiso_ejecutar_DB_FAME==1){
				    ...
				    #Desactivamos estos usuarios.
                    EjecutaQuery("UPDATE c_usuario SET fg_scf_revisado=0  WHERE  fg_scf='1' AND fl_perfil_sp=".PFL_ESTUDIANTE_SELF."  ");
            ...
            if(($upload_type=='teacher')||($upload_type=='Teacher')||($upload_type=='TEACHER')){
                ...
                #Limpiamos la revisio de la carga esto para hacer match y sabar cuantos registros elimnados hay.
                EjecutaQuery("UPDATE c_usuario SET fg_scf_revisado=0 WHERE fg_scf='1' AND fl_perfil_sp=".PFL_MAESTRO_SELF." ");
                
                GeneraLog($file_name_txt,date('F j, Y, g:i a')."#Limpiamos la revisio de la carga esto para hacer match y sabar cuantos registros elimnados hay..UPDATE c_usuario SET fg_scf_revisado=0 WHERE fg_scf='1' AND fl_perfil_sp=".PFL_MAESTRO_SELF."  ");
                ...
                if ($file = fopen($ruta_completa_archivo, "r")){
                    ...
                    while ($data = fgetcsv ($file, 0, ",")){
                        ...
                        if ($data[0]=='Teacher ID' || $data[1]=='Username' || $data[3]=='First Name' || $data[4]=='Last Name' || $data[5]=='School' || $data[6]=='School ID' || $data[6]=='Email') {
                            ...
                            if(!empty($teacher_id)){
                                ...
                                #Verificamos el isntitutio
                                $Query='SELECT fl_instituto,fl_usuario_sp FROM c_instituto WHERE school_id="'.$school_number.'" AND fg_scf="1" ';
                                $rop=RecuperaValor($Query);
                                ...
                                if($primeracarga==1){
                                    ...
                                    $Query ='SELECT ds_login,ds_alias,ds_nombres,ds_apaterno,ds_email,a.teacher_id,fg_scf,b.fl_usuario  
                                                     ...
                                                     AND b.ds_login="'.$username.'" AND ds_nombres="'.$first_name.'" AND ds_apaterno="'.$last_name.'" AND ds_email="'.$email.'" AND fg_scf=1 ';
                                    $row=RecuperaValor($Query);
                                    ...
                                    if(!empty($row[0])){
                                        ...
                                        EjecutaQuery('UPDATE c_usuario SET fg_scf_revisado=1 WHERE fl_usuario='.$fl_maestro_sp.' AND fl_perfil_sp='.PFL_MAESTRO_SELF.' AND fg_scf="1" ');
                                        
                                        GeneraLog($file_name_txt,date('F j, Y, g:i a').'UPDATE c_usuario SET fg_scf_revisado=1 WHERE fl_usuario='.$fl_maestro_sp.' AND fl_perfil_sp='.PFL_MAESTRO_SELF.' AND fg_scf="1" ');
                                        ...
                                        $Query ='SELECT ds_login,ds_alias,ds_nombres,ds_apaterno,ds_email,a.teacher_id,fg_scf,b.fl_usuario,b.fl_instituto  
                                                     ...
                                                     AND b.ds_login="'.$username.'" AND fg_scf=1  ';
                                        $row=RecuperaValor($Query);      
                                        $username_db=$row['ds_login'];
                                        $teacher_id_db=$row['teacher_id'];
                                        $fl_usuario_db=$row['fl_usuario'];
                    ...
                    #Revisamos cuanto no fueron porcesados de los existentes en FAME Y determinamos que fuweron eliminados.
                    $Query="SELECT COUNT(*)FROM c_usuario WHERE fg_scf_revisado=0 AND fg_scf='1' AND fg_activo='1' AND fl_perfil_sp=".PFL_MAESTRO_SELF."  ";
                    $ro=RecuperaValor($Query);
                    $deleted_count_teacher=$ro[0];

                    #Desactivamos estos institutos.
                    EjecutaQuery("UPDATE c_usuario SET fg_activo='0' WHERE fg_scf_revisado=0 AND fg_scf='1' AND fl_perfil_sp=".PFL_MAESTRO_SELF."  ");
                            ...
                            if(!empty($teacher_id)){
                                ...
                                if($primeracarga==1){
                                    ...
                                    if(!empty($row[0])){
                                        ...
                                        EjecutaQuery('UPDATE c_usuario SET fg_activo="1", fg_scf_revisado=1 WHERE fl_usuario='.$fl_maestro_sp.' AND fl_perfil_sp='.PFL_MAESTRO_SELF.' AND fg_scf="1" ');
