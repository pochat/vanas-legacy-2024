...
//	$client = $aws->get('Ses');
	...
	$Query2="SELECT  a.fl_instituto,a.fl_usuario_sp,b.fe_periodo_inicial, b.fe_periodo_final,a.fg_activo  
                FROM c_instituto a
                JOIN k_current_plan b ON b.fl_instituto=a.fl_instituto ";
	$Query2.="WHERE 1=1 AND a.fl_instituto <> 1  AND a.fg_activo='1'  AND a.fg_tiene_plan='1' ";
	$rs2 = EjecutaQuery($Query2);
	while($row2=RecuperaRegistro($rs2)){
	      $fl_instituto=$row2[0];
	      $fl_usuario=$row2[1];
		  $fe_creacion=$row2[2];
		  $fe_final_vigencia=$row2[3];
