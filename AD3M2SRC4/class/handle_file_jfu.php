...
// http://php.net/manual/en/features.file-upload.errors.php
    ...
    function __construct($options = null) {
        ...
        $this->options = array(
            ...
            'type' => '',
            'size' => 0,
            'fileDir' => ROOT . DS . 'public' . DS . 'media' . DS,
            'uploadUrl' => BASE_PATH . '/public/media',
            'deleteUrl' => '',
            'deleteType' => 'DELETE',
            'thumbnailUrl' => '', 
            'extention' => '.cid',
...
//print_r($options);
        ...
        if ($this->options['name'] == '') {
            $this->options['name'] = $this->friendfyUrl($file['name']);
        } else {
            $this->options['name'] = $this->friendfyUrl($this->options['name']);
        }
        ...
        if (move_uploaded_file($file['tmp_name'], $this->options['fileDir'] . DS . $this->options['name'])) {
            return $this->getResponseUpload($this->options);
        } else {
...
// Remove delimiter from ends
    ...
    public function friendfyUrl($str = '') {
