<?php
  ...
  EjecutaQuery("UPDATE k_criterio_programa SET no_valor = $value WHERE fl_criterio = $pk AND fl_programa = $name");
  
  $valida = RecibeParametroNumerico('valida');  
  $cle = RecibeParametroNumerico('cle');  
  
  if($valida == 1){
    $suma = RecuperaValor("SELECT SUM(no_valor) FROM k_criterio_programa WHERE fl_programa = $cle");
    echo $suma[0];
  }
