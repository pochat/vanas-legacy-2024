...
<script>
  ...
  if(!empty($fl_term_ini)) {
    for($i = 1; $i <= 4; $i++) {
     ...
     #SEP 06 2020  esta ejecutaba y obligaba a que se eliminaran los k_term_pagos y se perdia la relacion en k_alumno_pago se comenta , si surge error hay que descomentarlos. 
     // EjecutaQuery("DELETE FROM k_term_pago WHERE fl_term=$fl_term_ini AND no_opcion=$i AND no_pago>$no_pagos  ");
