<?php
  ...
  if(!empty($registros)){
    ...
    for($i=0;$row=RecuperaRegistro($rs);$i++) {
      echo "
      <tr>
        <td><a href=\"javascript:otro('".PATH_MODULOS."/campus/students_frm.php',$row[0]);\"'>".$row[2]."</a></td>
        <td><a href=\"javascript:otro('".PATH_MODULOS."/campus/students_frm.php',$row[0]);\"'>".$row[1]."</a></td>
        <td><a href=\"javascript:otro('".PATH_MODULOS."/campus/students_frm.php',$row[0]);\"'>".$row[3]."</a></td>
      </tr>";
    }
    Forma_Tabla_Fin();
  }
  ...
  if($permiso == PERMISO_DETALLE)
    $fg_guardar = ValidaPermiso(123, PERMISO_MODIFICACION);
  else
    $fg_guardar = True;
