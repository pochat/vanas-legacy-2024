...
<script>
  ...
  if(!empty($_FILES['archivo_f']['tmp_name'])) {
    $ruta = SP_IMAGES;
    $nb_flash_default = $_FILES['archivo_f']['name'];
    move_uploaded_file($_FILES['archivo_f']['tmp_name'], $ruta."/".$nb_flash_default);
  }
  if(!empty($_FILES['tr_archivo_ft']['tmp_name'])) {
    $ruta = SP_IMAGES;
    $tr_flash_default = $_FILES['tr_archivo_ft']['name'];
    move_uploaded_file($_FILES['tr_archivo_ft']['tmp_name'], $ruta."/".$tr_flash_default);
  }
  ...
  if(!empty($clave)) {
    ...
    $Query .= "fg_multiple='$fg_multiple', fg_tipo_orden='$fg_tipo_orden', fl_flujo=$fl_flujo, nb_flash_default='$nb_flash_default', ";
    $Query .= "tr_flash_default='$tr_flash_default' ";
