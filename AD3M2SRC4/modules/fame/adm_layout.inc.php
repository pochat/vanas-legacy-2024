...
function ArmaMenu( ) {
  ...
  # Tes de iconos
  $iconos = array(1 =>"fa-graduation-cap","fa-institution", " fa-cubes", "fa-folder","fa-gears", "fa-unlock-alt ", "fa-archive");
  # Prepara menu
  $menu = '
  <!-- #NAVIGATION -->
...
function Forma_Termina($p_guardar=False, $p_url_cancelar='', $p_etq_aceptar=ETQ_SALVAR, $p_etq_cancelar=ETQ_CANCELAR, $p_click_cancelar='') {
  ...
  echo "
    ...
    echo "
          ...
          if($p_guardar)
            echo "<a class='btn btn-primary btn-circle btn-xl' title='".$p_etq_aceptar."' name='aceptar' onClick='javascript:document.datos.submit();'><i class='fa fa-check'></i></a>&nbsp;";
    echo "<a class='btn btn-default btn-circle btn-xl' title='".$p_etq_cancelar."' name='aceptar' onClick=\"$click_cancelar\"><i class='fa fa-times'></i></a>
          </div>         
        </footer>
      </form>
    </div>
...
function Forma_Tabla_Ini($p_ancho, $p_tit=array(), $p_ancho_col=array(), $p_nombre='', $p_tresponsive=True) {
  ...
  if($tot > 0) {
    ...
    for($i = 0; $i < $tot; $i++) {
      ...
      //echo "
      //<th width='$p_ancho_col[$i]' class='text-align-".$align."'>$enc</th>";
      echo "
      <th class='text-align-".$align."' width='$p_ancho_col[$i]' style='background-color:#0092dc;' >$enc</th>";
    }
    echo "    
    </tr>
    </thead>";
...
function Forma_CampoInfo($p_prompt, $p_texto, $etq_align='right', $col_sm_etq='col-sm-4', $col_sm_etq='col-sm-4') {
...
function Forma_CampoPreviewUpload($p_prompt, $p_nombre, $p_valor, $p_ruta, $p_requerido, $p_archivo, $p_limpiar=True, $p_size, $p_error, $p_accept, $p_maxlength, $etq_align='right', $col_sm_etq='col-sm-4', $col_sm_cam='col-sm-4'){
 ...
 echo "
              ...
              CampoArchivo($p_archivo, $p_size, $ds_clase, $ds_accept, $p_maxlength);                                    
  echo "      </i>
              </span>
            </span>
          </div>
  ...
  echo "<div class='row'>";
    ...
    echo "<label class='col col-sm-12 control-label text-align-left'>";
      echo "<strong>{$p_titulo}</strong>";
        ...
        $ds_respuesta = str_texto($row[0]);  
        $fg_posicion_img = str_texto($row[1]);  
