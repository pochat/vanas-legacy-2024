<?php
 ...
 # }
  ...
  $fe_periodo = RecibeParametroHTML('fe_periodo');
	$tot_aut_nor = RecibeParametroNumerico('tot_aut_nor');
  $mn_total_aut = 0;
  for($i=0;$i<$tot_aut_nor;$i++){
  ...
  # Clases globales
  $tot_aut_cg = RecibeParametroNumerico('tot_aut_cg');
  $mn_total_cg = 0;
  for($i=0;$i<$tot_aut_cg;$i++){
    $fl_clase_global[$i] = RecibeParametroNumerico('fl_clase_global'.$i);
    $mn_cglobal_fee[$i] = RecibeParametroHTML('mn_cglobal_fee'.$i);
    $fl_clase_cg[$i] = RecibeParametroNumerico('fl_clase_cg'.$i);
    if($mn_cglobal_fee[$i])
        $mn_subtotal_aut_sp[$i] = $mn_cglobal_fee[$i] * 1;
    else
        $mn_subtotal_aut_sp[$i]=0;
    $subtract_cg[$i] = RecibeParametroBinario('subtract_cg'.$i);
    # Se debe obtener true si para insertar el monto
    if($subtract_cg[$i]==1){
        if($mn_subtotal_aut_sp[$i])
            $mn_subtotal_aut_sp[$i] = $mn_subtotal_aut_sp[$i];
        else
            $mn_subtotal_aut_sp[$i]=0;
    }else{ # Si es falso el monto sera cero
        $mn_subtotal_aut_sp[$i] = 0;
    }
    $mn_total_cg += $mn_subtotal_aut_sp[$i];
  }
  ...
  $mn_total = $mn_total_aut + $mn_total_cg +$mn_total_man;
  ...
  if($fg_error) {
    echo "<html><body><form name='datos' method='post' action='tarifas_frm.php'>\n";
...
<script>
  ...
  if(empty($fl_maestro_pago)){
    $fe_periodo = date("%Y-%m-%d", strtotime(date('d')."-".$fe_periodo));// tomamos el periodo y le agregamos el dia para guardalo
  ...
  if(!empty($fl_maestro_pago)){
    ...
    # Insertamos clases globales
    if(!empty($tot_aut_cg)){
      for($i=0;$i<$tot_aut_cg;$i++){
        # Si ya hay clases registradas para este maestro solo va actualizar los datos
        $search = RecuperaValor("SELECT fl_maestro_pago_det FROM k_maestro_pago_det WHERE fl_maestro_pago=$clave AND fl_grupo=$fl_clase_global[$i] AND ds_concepto=$fl_clase_cg[$i] AND fg_tipo='ACG' ");
        if(!empty($search[0]))
          $Query = "UPDATE k_maestro_pago_det SET fl_maestro_pago=$fl_maestro_pago WHERE fl_maestro_pago_det=$search[0]";
        else{
          $search1 = RecuperaValor("SELECT COUNT(*) FROM k_maestro_pago_det WHERE fl_maestro_pago=$fl_maestro_pago AND fl_grupo=$fl_clase_global[$i] AND ds_concepto=$fl_clase_cg[$i] AND fg_tipo='ACG'");
          if($search1[0])
            $Query  = "UPDATE k_maestro_pago_det SET mn_subtotal=".$mn_cglobal_fee[$i].", fg_subtract_class='".$subtract_cg[$i]."' WHERE fl_maelstro_pago=$fl_maestro_pago AND ds_concepto=".$fl_clase_cg[$i]."";
          else{
            $Query  = "INSERT INTO k_maestro_pago_det(fl_maestro_pago,fg_tipo,fl_grupo,ds_concepto,mn_tarifa_hr,no_horas, mn_subtotal, fg_subtract_class) ";
            $Query .= "VALUES($fl_maestro_pago,'ACG',$fl_clase_global[$i],'$fl_clase_cg[$i]',$mn_cglobal_fee[$i],'1',$mn_subtotal_aut_sp[$i], '$subtract_cg[$i]') ";
          }
        }
        EjecutaQuery($Query);
      }
