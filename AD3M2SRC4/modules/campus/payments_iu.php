<?php
  ...
  $ds_frecuencia = RecibeParametroHTML('ds_frecuencia');
  $tax_mn_cost = RecibeParametroFlotante('tax_mn_cost');
  ...
  if(!empty($fg_realizar)) {
    # Prepara fechas en formato para insertar
      if(!empty($fe_fecha))
      {
          $fe_fecha_email = "".ValidaFecha($fe_fecha)." ".date('H:i:s')."";
    ...
    if(!empty($fg_pago)) {
      if(empty($fg_app_frm)){
        $Query  = "INSERT INTO k_alumno_pago (fl_alumno, fl_term_pago, cl_metodo_pago, fe_pago, mn_pagado, ds_comentario, ds_cheque, mn_late_fee,tax_mn_cost) ";
        $Query .= "VALUES ($clave, $fl_term_pago, $cl_metodo_pago, $fe_fecha, $mn_pago, '$ds_comentario', '$ds_cheque', $mn_late_fee,$tax_mn_cost) ";



      }
      else{
        $Query  = "INSERT INTO k_ses_pago (cl_sesion, fl_term_pago, cl_metodo_pago, fe_pago, mn_pagado, ds_comentario, ds_cheque, mn_late_fee,tax_mn_cost) ";
        $Query .= "VALUES ('$cl_sesion', $fl_term_pago, $cl_metodo_pago, $fe_fecha, $mn_pago, '$ds_comentario', '$ds_cheque', $mn_late_fee,$tax_mn_cost) ";
    ...
    if($aplications){
        #Recupermos datos del estuduiante.
        $QueryStudent="SELECT ds_email from k_ses_app_frm_1 where cl_sesion='$cl_sesion' ";
    }else{
        #Recupermos datos del estuduiante.
        $QueryStudent="SELECT ds_email from c_usuario where cl_sesion='$cl_sesion' ";
    }
    $Querys="SELECT fl_sesion FROM c_sesion WHERE cl_sesion='$cl_sesion' ";
    $rows=RecuperaValor($Querys);
    $fl_sesion=$rows['fl_sesion'];

    $rowstu=RecuperaValor($QueryStudent);
    $email=$rowstu['ds_email'];

    switch($cl_metodo_pago){

        case '2':
            $payment_method="Payment Manual";
            break;
        case '3':
        ...
        case '4':
            ...
            break;
        case '5':
            $payment_method="Wire Transfer/Deposit";
            break;
        case '6':
            $payment_method="Cash";
            break;
    ...
    $nb_template="Receipt of payment ";

    # generador de documento
    $ds_header = genera_documento($fl_sesion, 1, 216);
    $ds_cuerpo = genera_documento($fl_sesion, 2, 216);
    $ds_footer = genera_documento($fl_sesion, 3, 216);
    $message = utf8_decode($ds_header).utf8_decode($ds_cuerpo).utf8_decode($ds_footer);

	if($ds_frecuencia<>"Full Payment"){
		$ds_frecuencia= $ds_frecuencia." payment";
	}
    ...
    $message = str_replace("#amount#", "$".$mn_pago, $message);
    $message = str_replace("#payment_frequency#", "".$ds_frecuencia, $message);
    $message = str_replace("#payment_method#", "".$payment_method, $message);
    $message = str_replace("#payment_comments#", $ds_cheque." ".$ds_comentario, $message);


    #send email notificacion pago
    Mailer($email,$nb_template,$message,'','','');
