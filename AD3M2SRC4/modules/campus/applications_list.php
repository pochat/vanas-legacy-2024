<?php
  ...
  $Query .= ", ds_email, ds_number, ds_alt_number,fe_ultmod,fg_payment,fl_programa,cl_sesion,mn_tax_paypal,fl_pais,fl_estado,mn_app_fee,mn_tuition,mn_discount,fg_enrollment,comments FROM( ";
  ...
  $Query .= ",b.ds_email, b.ds_number, b.ds_alt_number,a.fe_ultmod,c.fg_payment,e.fl_programa,a.cl_sesion,a.mn_tax_paypal,b.ds_add_country fl_pais, b.ds_add_state fl_estado,c.mn_app_fee,c.mn_tuition,c.mn_discount,a.fg_enrollment,b.comments  ";
...
?>
{
    ...
    for($i=1;$row=RecuperaRegistro($rs);$i++){
     ...
     # This Variable was not present and generate an error that is now corrected
     ...
     $Query="SELECT link_contract from k_app_contrato where cl_sesion='$cl_sesion' ";
     $rowp=RecuperaValor($Query);
     $link_contract=$rowp[0];
     ...
     if($fg_payment=='C'){
      ...
      if(!empty($regis)){
        for($j=0;$rowd=RecuperaRegistro($rsd);$j++) {
          ...
          # Variables added since are needed on $duplicados
          $msj1=isset($rowd[0])?$rowd[0]:NULL;
          $msj2=isset($rowd[1])?$rowd[1]:NULL;
          $msj3=isset($rowd[2])?$rowd[2]:NULL;
          $msj4=isset($rowd[3])?$rowd[3]:NULL;
          $duplicados .= "<strong>".$msj2."</strong><br/><small class='text-muted'><i>".$msj3.", ".$msj4.". Grade: ".$msj1."</i></small><br/>";
        }
      }
      // Preferencias
	  ...
	  $Query="SELECT mn_pagado,fg_stripe,convenience_fee FROM c_sesion WHERE cl_sesion='$cl_sesion' ";
	  $ro=RecuperaValor($Query);
	  $mn_pagado_app_fee=isset($ro['mn_pagado'])?$ro['mn_pagado']:NULL;
      $fg_stripe=$ro['fg_stripe'];
      $mn_convenience_fee_app_fee=$ro['convenience_fee'];
	  ...
	  if($fg_tax_rate==1){
		  ...
		  if(($fl_pais==38)||($fl_pais==226)){
			 ...
			 if($mn_convenience_fee_app_fee){
                 $info_app_fee.="  <b>Convenience fee:</b> $".number_format($mn_convenience_fee_app_fee,2);
             }
             ...
             if($mn_convenience_fee_app_fee){
                 ...
                 $mn_convenience_fee_tuition=($mn_tuition+$mn_tax_tuiton)*$convenience_fee_percentage/100;

                 $info_tuition.="  <b>Convenience fee:</b>$ ".number_format($mn_convenience_fee_tuition,2);
             }
             ...
             $tot_tuiton=$mn_tuition+$mn_tax_tuiton;
             $info_tuition="<b>".ObtenEtiqueta(599)."</b>:$".number_format($mn_tuition,2)."+ $".number_format($mn_tax_tuiton,2)."=$".number_format($tot_tuiton,2);
            // if($mn_convenience_fee){
              //   $info_tuition.="  <b>Convenience fee:</b>$ ".number_format($mn_convenience_fee,2);
            // }
             $tot_tot=$mn_total_app+ $tot_tuiton;
    ...
    #Verifica si tiene ID asignado y si no lo inserta.
    $Queryid="SELECT id_alumno,fl_pais_campus,fl_sesion,fg_scholarship FROM c_sesion WHERE cl_sesion='$cl_sesion' ";
    $rowid=RecuperaValor($Queryid);
    $fl_sesion=$rowid['fl_sesion'];
    $fg_scholarship=!empty($rowid['fg_scholarship'])?"YES":"NO";
    if(empty($rowid[0])){

        $Queryid="UPDATE c_sesion SET  id_alumno='$ds_login' WHERE cl_sesion='$cl_sesion' ";
        EjecutaQuery($Queryid);
    ...
    $fl_pais_campus=$rowid['fl_pais_campus'];
    $etq_pago=$row[15];
    if($fl_pais_campus==226){
        ...
        if($row[15]=='Paypal Manual'){
            $etq_pago="Stripe Manual";
        }
    ...
    switch ($fl_pais_campus) {
        ...
        case '199':
                ...
                $etq_pago = ($row[15]=='Paypal')?"Stripe":"Stripe Manual";
            break;
        case '73':
    ...
    if(!empty($fg_enrollment)){
        $enrollment="<span class='label label-success'>Ready to enroll</span>";
    }else{
        $enrollment="";
    }
    ...
    $link_payment="https://campus.vanas.ca/app_form/index.php?c=A99".$fl_sesion."22f0&co=".$fl_pais_campus."&p=19&cd=1";
      ...
      echo '
        {
            ...
            "name": "<a href=\'javascript:Envia(\"applications_frm.php\",'.$row[0].');\'><b>'.str_texto($row[1]).'</b><br><small class=\'text-muted\'><i>'.str_texto($row[2]).'&nbsp;'.str_texto($row[3]).'</i></small></a><br><small class=\'text-muted\'>'.$ds_login.'</small><br><br><small><i>'.$etq_campus.'</i></small>",
            "country": "<td>'.str_texto($row[5]).'<br><small class=\'text-muted\'><i>'.$row[6].'</i></small><br><br>'.$enrollment.'<br>'.$txt_fg_disability.'</td>",
            ...
            "portafolio": "<a href=\''.$row[14].'\' target=\'_blank\'>'.$row[14].'</a><br><br><textarea class=\'form-control custom-scroll\' id=\'comments_'.$row[0].'\' name=\'comments_'.$row[0].'\' cols=\'50\' rows=\'3\' disabled>'.$comments.'</textarea>&nbsp;<button id=\'btn_edit'.$row[0].'\' class=\'btn btn-xs btn-default\' onclick=\'editComments('.$row[0].')\' style=\'\'>Edit</button><button id=\'btn_save'.$row[0].'\' class=\'btn btn-xs btn-default\' style=\'display:none;\' onclick=\'saveComments('.$row[0].')\' >Save</button>",
            "firt_payment": "<span class=\'label label-'.$firt_color.'\'>'.$row[12].'</span>",
            "metodo": "<span class=\'label label-'.$color.'\'>'.$etq_pago.'</span><br>$'.number_format($tot_tot,2).' <br> '.$mn_tax_.'",
            ...
            "link_contract":"<a href=\"'.$link_contract.'\">'.$link_contract.'</a><br> <span>link payment: </span><a href=\"'.$link_payment.'\" target=\'blank\'>'.$link_payment.'</a> ",
            "information":"<div class=\'col col-sm-12 col-xs-12 col-md-12\'>'
			  ...
			  . '<div class=\'col col-sm-4 col-md-4 col-xs-4\'><strong>ID:</strong><small ><br>'.$ds_login.' </div>'
        . '<div class=\'col col-sm-4 col-md-4 col-xs-4\'><strong>Scholarship:</strong><small ><br>'.$fg_scholarship.' </div>'
              . '</div>"
        }';
      if($i<=($registros-1))
        echo ",";
