<?php
  ...
  if(!$fg_error) { // Sin error, viene del listado
    ...
    if(empty($ds_ruta_video)){
      $roww = RecuperaValor("SELECT nb_archivo FROM k_vid_news_temp WHERE fl_usuario=$fl_usuario_mod AND ds_tipo='N'");
      $ds_ruta_video = $roww[0];
    }
  ...
  Forma_Espacio( );
  #### Inicamos video ######## 
  $ruta_img = ObtenConfiguracion(116)."/vanas_videos/campus/news/video_".$clave."/video_".$clave."_hd/img_1.png";
  if(!empty($ds_ruta_video))
    $fg_actualizar = 1;
  else
    $fg_actualizar = 0;
  # Buscamos registro para ver si esta actulizando el video    
  if(!empty($clave))
    $rowe = RecuperaValor("SELECT COUNT(*) FROM k_vid_news_temp WHERE fl_usuario=".$fl_usuario_mod." AND fl_clave=".$clave." AND ds_tipo='N'");
  else
    $rowe = RecuperaValor("SELECT COUNT(*) FROM k_vid_news_temp WHERE fl_usuario=".$fl_usuario_mod." AND ds_tipo='N'");
  if($fg_error==1 && $rowe[0]>0)
    $fg_errorr = 1;
  else
    $fg_errorr = 0;
    
  $name_pa = array("clave", "usuario", "fg_actualizar");
  $valores_pa = array($clave, $fl_usuario_mod, $fg_actualizar);
  Forma_DropzoneVideos($clave, $ds_ruta_video, '', 'dropschoolnews', 'vid_schoolnew.php', ".mov, .mp4, .flv", $name_pa, $valores_pa, false, $ruta_img, "new_progreso.php", "","../programs/preview_flv.php", "&fg_tipo=news", $fg_errorr);
