...
<?php
  ...
  $nuevo = $_POST['nuevo'];
  $actual = $_POST['actual'];
  // echo "criterio:".$criterio."<br>actual:".$actual."<br>nuevo:".$nuevo;
  // exit;
  # Verifica que exista una sesion valida en el cookie y la resetea 1 hra
  ValidaSesion( );
  
  if($actual==6)   # Si se realiza busqueda avanzada
    $Query = Query_Adv_Search($nuevo,$criterio);
  else
    $Query = Query_Completo($criterio, $actual);
    
  function Query_Adv_Search($p_criterio,$criterio_search='') {
    

      #Verifica si es fecha.
      $date=explode("-",$p_criterio);
      $dat1=$date[0];
      $dat2=$date[1];
      if((!empty($dat1))&&(!empty($dat2))){
          $p_criterio=ValidaFecha($p_criterio);
      }




    $Query  = "SELECT fl_usuario, ds_login 'Login', ";
    $Query .= "ds_nombres 'Name(s)', ";
    ...
    //$Query .= "fe_ultacc '".ObtenEtiqueta(112)."', ";
    $Query .= "CASE WHEN fg_international='Yes' THEN '".ObtenEtiqueta(NO_ETQ_SI)."' ";
    ...
    $Query .= "fg_study_permit '".ObtenEtiqueta(1025)."', ";
    $Query .= "ds_citizenship 'Citizenship', aboriginal '".ObtenEtiqueta(1027)."', health '".ObtenEtiqueta(1032)."', disablity '".ObtenEtiqueta(1778)."' ";
    $Query .= ",advisor,fl_programa,fl_periodo ";
    $Query .= "FROM (";
    $Query .= "
    ...
    ds_website,(SELECT nb_programa FROM c_programa e WHERE e.fl_programa=f.fl_programa LIMIT 1) nb_programa,
    (SELECT CONCAT(h.ds_nombres, ' ', h.ds_apaterno) FROM k_alumno_grupo k, c_grupo gpo, c_usuario h WHERE k.fl_grupo=gpo.fl_grupo AND gpo.fl_maestro=h.fl_usuario AND k.fl_alumno=a.fl_usuario AND k.fg_grupo_global='0' LIMIT 1 ) ds_profesor,
    (SELECT nb_grupo FROM c_grupo n, k_alumno_grupo o WHERE n.fl_grupo=o.fl_grupo AND o.fl_alumno=a.fl_usuario  AND o.fg_grupo_global='0' LIMIT 1 ) nb_grupo,
    (SELECT no_grado FROM k_term b, k_alumno_grupo d, c_grupo m WHERE b.fl_term=m.fl_term AND d.fl_grupo=m.fl_grupo AND d.fl_alumno=a.fl_usuario AND d.fg_grupo_global='0' LIMIT 1) no_grado, 
    ...
    ds_add_city, 
		CASE 
		WHEN ds_add_state REGEXP '^-?[0-9]+$' = '0' THEN ds_add_state
		ELSE ( SELECT ds_provincia FROM k_provincias WHERE fl_provincia=ds_add_state)
  END 
	 ds_add_state,
    ...
    YEAR(fe_nacimiento) ye_fe_nacimiento, YEAR(fe_alta) ye_fe_alta, YEAR(fe_ultacc) ye_fe_ultacc, YEAR(f.fe_ultmod) ye_fe_ultmod, 
    ...
    (SELECT CASE WHEN app2.fg_study_permit='1' THEN '".ObtenEtiqueta(NO_ETQ_SI)."' ELSE '".ObtenEtiqueta(NO_ETQ_NO)."' END FROM k_app_contrato app2 WHERE app2.cl_sesion=a.cl_sesion AND no_contrato='1') fg_study_permit,
    (SELECT CASE WHEN app3.fg_international='1' THEN app3.ds_citizenship ELSE 'Canada' END FROM k_app_contrato app3 WHERE app3.cl_sesion=a.cl_sesion AND app3.no_contrato='1') ds_citizenship,
    CASE WHEN fg_disability='1' THEN ds_disability ELSE '".ObtenEtiqueta(NO_ETQ_NO)."' END disablity,
    (SELECT CASE WHEN app4.fg_aboriginal='1' THEN app4.ds_aboriginal ELSE '".ObtenEtiqueta(NO_ETQ_NO)."' END FROM k_app_contrato app4 WHERE app4.cl_sesion=a.cl_sesion AND app4.no_contrato='1') aboriginal,
    (SELECT CASE WHEN app5.fg_health_condition='1' THEN app5.ds_health_condition ELSE '".ObtenEtiqueta(NO_ETQ_NO)."' END FROM k_app_contrato app5 WHERE app5.cl_sesion=a.cl_sesion AND app5.no_contrato='1') health
    , (SELECT CONCAT( userr.ds_nombres, ' ', userr.ds_apaterno ) FROM c_usuario userr where userr.fl_usuario=f.cl_recruiter  ) advisor,ds_add_state ds_add_state_2
    ,f.fl_programa,f.fl_periodo
    FROM (c_usuario a, c_alumno c, k_ses_app_frm_1 f, c_perfil b) LEFT JOIN k_pctia j ON (a.fl_usuario=j.fl_alumno)
    LEFT JOIN c_sesion ses ON(ses.cl_sesion=a.cl_sesion)
    ...
    if(!empty($criterio_search)){
        
        $Query .=" 
              AND ( ds_login LIKE '%$criterio_search%' OR  
	  	      ds_nombres LIKE '%$criterio_search%' OR  
			  ds_amaterno LIKE '%$criterio_search%' OR  
			  ds_apaterno LIKE '%$criterio_search%' OR  
			  ds_email LIKE '%$criterio_search%' OR 
			  nb_programa LIKE '%$criterio_search%' OR  
			  nb_grupo LIKE '%$criterio_search%' OR  
			  ds_add_city LIKE '%$criterio_search%' OR   
			  ds_add_state_2 LIKE '%$criterio_search%' OR 
	  		  ds_pais LIKE '%$criterio_search%' 
			   ) 
            ";
    }
    ...
    $Query .= "ORDER BY principal.fe_alta DESC";
    ...
    while($row = RecuperaRegistro($rs)) {
        for($i = 1; $i < $tot_campos; $i++){
         ...
         /*   if($i==40){
                ...
                $Querysem  = "SELECT no_semanas ";
                $Querysem .= "FROM k_programa_costos a ";
                $Querysem .= "WHERE a.fl_programa = $row[66]";
                $rowsem = RecuperaValor($Querysem);
                $no_semanas = $rowsem[0];
                
                $Queryperio="SELECT fe_inicio FROM c_periodo WHERE fl_periodo=$row[67] ";
                $rowperio = RecuperaValor($Queryperio);
                $fe_inicio = $rowperio['fe_inicio'];
                ...
                $QueryUpdate  = "UPDATE k_pctia set fe_fin='$fe_fin' where fl_alumno=$row[0] ";
                EjecutaQuery($QueryUpdate);


                fwrite($archivo, str_replace(",", " ", getStrParaCSV(str_ascii(DecodificaEscogeIdiomaBD($fe_fin)))).",");
  ...
  function Query_Completo($p_criterio, $p_actual) {
    ...
    while($row = RecuperaRegistro($rs)) {
      ...
      $Query  = "SELECT a.fl_programa, ds_number, ds_alt_number, ds_add_number, ds_add_street, ds_add_city, ds_add_state, ds_add_zip, ";
