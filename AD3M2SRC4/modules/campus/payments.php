<?php
  ...
  $fg_export_cvs=RecibeParametroNumerico('fg_export_cvs');
  ...
  IFNULL(unearned,'0.00') '".ObtenEtiqueta(742)."', IF(ganados='0/0','(To be paid)',ganados )  '".ObtenEtiqueta(743)."',ds_provincia 'State/Province' ";
  ...
  $QueryTST .= "	case when zz.ds_add_state REGEXP '^[0-9]+$'
			 then
				(SELECT ds_provincia FROM k_provincias za where za.fl_provincia=zz.ds_add_state)
				ELSE zz.ds_add_state END ds_provincia ";
  ...
  $QueryTST .= " JOIN k_ses_app_frm_1 zz ON zz.cl_sesion= b.cl_sesion ";
  ...
  # App fee students
  $QueryAPP  = "(SELECT CASE WHEN fg_inscrito='1' THEN (SELECT min(fl_usuario) FROM c_usuario m WHERE m.cl_sesion=a.cl_sesion) END clave, ";
  ...
  $QueryAPP .= ",	case when b.ds_add_state REGEXP '^[0-9]+$' then
				(SELECT z.ds_provincia FROM k_provincias z where z.fl_provincia=b.ds_add_state)
				ELSE b.ds_add_state end ds_provincia ";
  ...
  $QueryTNST .= ",	case when g.ds_add_state REGEXP '^[0-9]+$' then
				(SELECT z.ds_provincia FROM k_provincias z where z.fl_provincia=g.ds_add_state)
				ELSE g.ds_add_state end ds_provincia ";
  ...
  $QueryAPPN .= ",	case when b.ds_add_state REGEXP '^[0-9]+$' then
				(SELECT z.ds_provincia FROM k_provincias z where z.fl_provincia=b.ds_add_state)
				ELSE b.ds_add_state end ds_provincia ";
  ...
  if($fg_export_cvs=='2'){
       $Query3 .= $Query . "ORDER BY  ds_nombre,fe_pagado,ds_nombre,nb_programa, no_grado, CAST(no_pago AS UNSIGNED) ASC ";
  }else{
 ...
 $total = number_format(round($row_paid[0]+$row_todepaid[0]), 2, '.', ',');
  ...
  if($fg_export_cvs=='1'){
      require 'payments_export_cvs.php';
  }
  if($fg_export_cvs=='2'){
      require 'payments_export_qbo_cvs.php';
  }
