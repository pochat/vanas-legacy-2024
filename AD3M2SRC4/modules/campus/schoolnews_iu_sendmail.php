<?php
  ...
  $ds_template = html_entity_decode($schoolnews_template[0].$schoolnews_template[1].$schoolnews_template[2]);

  # Create a DOM object
  $ds_template_html = new simple_html_dom();

  # Prepare the right abstract text
  $ds_abstract = html_entity_decode($ds_resumen);
  ...
  if($fg_maestros == "1") {
    $rs = EjecutaQuery("SELECT b.ds_nombres AS us_fname, b.ds_email FROM c_maestro a LEFT JOIN c_usuario b ON b.fl_usuario=a.fl_maestro WHERE b.fg_activo='1' AND b.fl_perfil=".PFL_MAESTRO." ");
    while($row = RecuperaRegistro($rs)){
      ...
      Mailer($ds_email,$ds_email_title,$ds_template_html,'','','','',true);

    }
  }
  if($fg_alumnos == "1") {
    $rs = EjecutaQuery("SELECT b.ds_nombres AS us_fname, b.ds_email FROM c_alumno a LEFT JOIN c_usuario b ON b.fl_usuario=a.fl_alumno WHERE b.fg_activo='1' AND b.fl_perfil=".PFL_ESTUDIANTE." ");
    while($row = RecuperaRegistro($rs)){
    	$us_fname = $row[0];
    	$ds_email = $row[1];
