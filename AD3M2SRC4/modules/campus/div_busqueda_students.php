...
if (!empty($opcion1)) {
    ...
    switch ($tipo) {
        ...
        <select name='filtro_$opc' id='filtro_$opc' class='select2' >";
      ...
      <div class='col col-sm-6'><label class='input'><input type='text' name='fe_dos_$opc' id='fe_dos_$opc' maxlength='10' class='datepicker hasDatepicker' value='$fecha2' placeholder='" . ObtenEtiqueta(642) . "'>" . Forma_Calendario("fe_dos_$opc")."</label></div>";
        ...
        default:
          ...
          <div class='col col-sm-6'><label class='select'>
            ...
            if($opcion == 'ds_add_state')
            {
                $Query  = "SELECT ds_provincia, fl_provincia FROM k_provincias WHERE fl_pais=38 ORDER BY ds_provincia";
                Forma_CampoSelectBD('nolabel', False, 'criterio_'.$opc.'', $Query, 'criterio_'.$opc.'', '', True, '', 'left', 'col col-sm-12', 'col col-sm-12');
                
                
            }else{
                echo"<label class='input'><input type='text' maxlength='250' size='30' class='form-control input-sm' name='criterio_$opc' id='criterio_$opc' value='$crit'></label>";
            }
            echo"</div>";
            break;
    }
