<?php
  ...
  //# $Query variable initialization to avoid errors
  //$Query="";
  ...
  //exit;
    ...
    //echo "campo:".$campo."tipo:".$tipo."filtro".$filtro."fecha1".$fecha1."fecha2".$fecha2."criterio".$criterio."<br/>";
		if (!empty($campo)){
			...
			switch ($tipo){
              ...
              $Query = "AND $campo LIKE 'M' ";		
				...
				case 2:
                    ...
                    $fecha1=strtotime('+0 day',strtotime($fecha1));
                    $fecha1= date('Y-m-d',$fecha1);

                    $fecha2=strtotime('+0 day',strtotime($fecha2));
                    $fecha2= date('Y-m-d',$fecha2);
          //$Query = "AND $campo >= STR_TO_DATE('$fecha1', '%d-%m-%Y') AND $campo <= STR_TO_DATE('$fecha2', '%d-%m-%Y') ";
                    $Query = "AND DATE_FORMAT($campo,'%Y-%m-%d')>='$fecha1' AND DATE_FORMAT($campo,'%Y-%m-%d') <='$fecha2' ";
                    break;
          ...
          switch ($filtro){
            case 1:
                if($campo=='ds_add_state'){
                    $Query = "AND ( $campo LIKE '%$criterio%' OR ds_add_state_2 LIKE '%$criterio%' )  ";
                }else{
          ...
          break;
        default:
          $Query = "fg_activo='1' OR fg_activo='0' ";
      }
      return $Query;
    }
  }
