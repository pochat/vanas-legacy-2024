<?php
  ...
  if(!$fg_error) { // Sin error, viene del listado
    if(!empty($clave)) { // Actualizacion, recupera de la base de datos
      $Query  = "SELECT nb_tema, no_orden, ds_ruta_imagen, no_posts ";
      $Query .= "FROM c_f_tema ";
      $Query .= "WHERE fl_tema=$clave";
      $row = RecuperaValor($Query);
      $nb_tema = str_texto($row[0]);
      $no_orden = $row[1];
      $ds_ruta_imagen = str_texto($row[2]);
      $no_posts = $row[3];
    }
  ...
  if($permiso == PERMISO_DETALLE)
    $fg_guardar = ValidaPermiso(84, PERMISO_MODIFICACION);
  else
    $fg_guardar = True;
