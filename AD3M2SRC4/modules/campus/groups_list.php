<?php
  ...
  $Query .= "(SELECT COUNT(1) FROM k_alumno_grupo f WHERE a.fl_grupo=f.fl_grupo) AS count,fg_grupo_global,a.fg_zoom ";
  ...
  $Query.="
      SELECT a.fl_grupo,''nb_programa,''nb_periodo,''no_grado,a.nb_grupo,''Teacher,
       (SELECT COUNT(1) 
      FROM k_alumno_grupo z WHERE a.fl_grupo=z.fl_grupo) AS count,fg_grupo_global,a.fg_zoom   
      FROM c_grupo a 
...
?>
{
    ...
    for($i=1;$row=RecuperaRegistro($rs);$i++) {
      ...
      $Query2="SELECT a.fl_alumno,b.ds_ruta_avatar, CONCAT(c.ds_nombres,' ',c.ds_apaterno)AS nombre FROM k_alumno_grupo a 
              ...
              WHERE a.fl_grupo=$fl_grupo AND c.fg_activo='1' ";
	  ...
	  if($fg_grupo_global==1){
          ...
          #Recupermos los teacher.
          $Queryt="	SELECT DISTINCT fl_maestro, CONCAT(b.ds_nombres,' ',b.ds_apaterno,' ',b.ds_amaterno)nombres FROM k_clase_grupo a 
		            JOIN c_usuario b ON b.fl_usuario=a.fl_maestro
		            WHERE fl_grupo=$fl_grupo  AND b.fg_activo='1' ";
          $rst=EjecutaQuery($Queryt);
          $nb_teacher="";
          for($x=1;$rowt=RecuperaRegistro($rst);$x++){
              
              $nb_teacher.=str_texto($rowt['nombres'])."<br>";
   ...
   //  if($fl_grupo<>282){    
    ...
    echo '
    {
        ...
        "grupo": "<a href=\'javascript:Envia(\"groups_frm.php\",\"'.$fl_grupo.'\");\'>'.$nb_grupo.' <br><small class=\'text-muted\'>'.$zoom.'</small></a> ",
