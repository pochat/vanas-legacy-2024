<?php
  ...
  if(!$fg_error) { // Sin error, viene del listado
    if(!empty($clave)) { // Actualizacion, recupera de la base de datos
      $row = RecuperaValor("SELECT nb_etiqueta, ds_etiqueta, ds_etiqueta_esp, ds_etiqueta_fra, tr_etiqueta FROM c_etiqueta WHERE cl_etiqueta=$clave");
      $nb_etiqueta = str_texto($row["nb_etiqueta"]);
      $ds_etiqueta = str_texto($row["ds_etiqueta"]);
      ...
      $tr_etiqueta = str_texto($row["tr_etiqueta"]);
    }
    ...
    $ds_etiqueta_esp = RecibeParametroHTML("ds_etiqueta_esp");
    $ds_etiqueta_fra = RecibeParametroHTML("ds_etiqueta_fra");
  ...
  Forma_Espacio( );
  Forma_CampoTexto(ETQ_NOMBRE, True, 'nb_etiqueta', $nb_etiqueta, 50, 50, $nb_etiqueta_err);
  Forma_CampoTexto(ETQ_DESCRIPCION, True, 'ds_etiqueta', $ds_etiqueta, 1000, 50, $ds_etiqueta_err);
  Forma_CampoTexto("Spanish", False, 'ds_etiqueta_esp', $ds_etiqueta_esp, 1000, 50);
  Forma_CampoTexto("French", False, 'ds_etiqueta_fra', $ds_etiqueta_fra, 1000, 50);
