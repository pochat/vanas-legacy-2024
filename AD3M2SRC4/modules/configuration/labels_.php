<?php
  ...
  if(!empty($criterio)) {
    switch($actual) {
      ...
      case 3: $Query .= "WHERE ds_etiqueta LIKE '%$criterio%' OR ds_etiqueta_esp LIKE '%$criterio%' OR ds_etiqueta_fra LIKE '%criterio%'"; break;
      default:
        $Query .= "WHERE cl_etiqueta LIKE '%$criterio%' OR nb_etiqueta LIKE '%$criterio%' OR ";
        $Query .= "ds_etiqueta LIKE '%$criterio%' OR ds_etiqueta_esp LIKE '%$criterio%' OR ds_etiqueta_fra LIKE '%$criterio%' ";
    }
  }
