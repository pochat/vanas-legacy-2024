...
//require_once 'adobeconnect/LicenciaAdobe.class.php';
...
class AdobeConnectClient {
  ...
  private $rootFolderId;
	...
	public function createMeeting($folder_id, $name, $date_begin, $date_end, $url) {
		...
		$estatus_duplicado = !empty($result['status']['invalid']['@attributes']['subcode'])?$result['status']['invalid']['@attributes']['subcode']:NULL;
		
		$data = array();
		
		//echo "Estatus duplicado: [$estatus_duplicado] <br>";
		
		if ($estatus_duplicado == 'duplicate')
			$data = $this->getMeeting($folder_id, $name);
		else {
			$data["estatus"] = "nuevo";
			$data["meeting_id"] = $result['sco']['@attributes']['sco-id'];
			$data["url-path"] = $url;
		}
	...
	public function getMeeting($folder_id, $name) {
		...
		$data["estatus"] = "existe";
		$data["meeting_id"] = $result['sco']['@attributes']['sco-id'];
		$data["url-path"] = $result['sco']['url-path'];	
  ...
  // Recibo el sco-id que es la clave de la sesion
    ...
    private function makeRequest($action, array $params = array()) {
		...
		// MDB Quite esta validacion para poder identificar las clases duplicadas
		/*
