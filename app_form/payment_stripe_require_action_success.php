...
$payment_intent_id=RecibeParametroHTML('payment_intent_id');
...
try {
    ...
    #recuperar informacion del pago y proceder al guardar los datos del pagos en DB.
    $Query="SELECT * FROM stripe_payment_intent WHERE payment_intent_id='$payment_intent_id' ";
    $row=RecuperaValor($Query);
    $cl_sesion=$row['cl_sesion'];
    $currency=$row['currency'];
    $calholder=$row['calholder'];
    $mn_app_fee_only=$row['mn_app_fee_only'];
    $charge_id=$row['charge_id'];
    $mn_tax_amount=$row['mn_tax_amount'];
    $ds_tax=$row['ds_tax_provincia'];
    $convenience_fee_app_fee=$row['convenience_fee'];
    $fl_term_pago=$row['fl_term_pago'];
    $mn_tuition=$row['mn_tuition'];
    $ds_transaccion=$row['ds_transaccion'];
    $convenience_fee_tuition=$row['convenience_fee_tuition'];
    $mn_tax_tuituion=$row['mn_tax_tuituion'];
    ...
    if ($charge->data[0]->paid == true) {
        ...
        #Recuperamos datos insertados previamente.
        ...
        EjecutaQuery("UPDATE c_sesion SET fg_app_1='1',fg_app_2='1',fg_app_3='1',fg_app_4='1'  WHERE cl_sesion='".$cl_sesion."'");
        EnviarEmailForms2($cl_sesion,false);

        $Query="SELECT ds_fname,ds_lname,ds_email  FROM k_ses_app_frm_1 WHERE cl_sesion='$cl_sesion'  ";
