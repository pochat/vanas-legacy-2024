...
class JFormFieldCombobox extends JFormFieldList
{
  var $_moduleName = '';
    ...
    return '<input type="text" name="'.$this->name.'" id="'.$this->id.'" value="'.$this->value.'" />'.
		    JHTML::_('select.genericlist',  $options, $this->id.'select', $class.' onchange="dojo.byId(\''.$this->id.'\').value = this.options[this.selectedIndex].value; this.selectedIndex=0;"', 'value', 'text', '', $this->id.'list');
	}
}
