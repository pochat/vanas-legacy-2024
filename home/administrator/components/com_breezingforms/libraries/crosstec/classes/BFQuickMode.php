			...
			if($mdata['type'] == 'page'){
				...
				switch($mdata['bfType']){
					...
					case 'bfFile':
						...
						if(isset( $mdata['flashUploader'] ) && $mdata['flashUploader']){
							...
							var bfFlashFileQueue".$mdata['dbId']." = {};
									...
									'script'         : '".JURI::root(true).(JRequest::getCmd('lang','') && JFactory::getConfig()->getValue('config.sef') ? '/'.JRequest::getCmd('lang','') : '')."/index.php',
					...
					case 'bfCalendar':
						...
						echo '<button id="ff_elem'.$mdata['dbId'].'_calendarButton" type="submit" class="bfCalendar" value="'.htmlentities(trim($mdata['value']), ENT_QUOTES, 'UTF-8').'"><span>'.htmlentities(trim($mdata['value']), ENT_QUOTES, 'UTF-8').'</span></button>'."\n";
						echo '</span>'."\n";
