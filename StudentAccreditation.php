<?php
    ...
    $fl_sesion = RecibeParametroNumerico('clave', True);
    ...
    // Get alumno data
    $Query="SELECT ds_login, ds_nombres, ds_apaterno, ds_amaterno, ".ConsultaFechaBD('fe_nacimiento', FMT_CAPTURA)." fe_nacimiento, cl_sesion, no_promedio_t,fe_alta 
              FROM c_usuario a, c_perfil b, c_alumno c 
              WHERE a.fl_perfil=b.fl_perfil 
              AND a.fl_usuario=c.fl_alumno 
              AND fl_usuario=$fl_user";
    ...
    $full_name = $ds_nombres.' '.$ds_amaterno.' '.$ds_apaterno;
    ...
    if($fl_pais_campus==226){#USA    
        if($fg_payment=='C'){
            $fg_tipo_curso=ObtenEtiqueta(2674);
        }
		$ds_campus="USA";
    }else{
        if($fg_payment=='C'){
            $fg_tipo_curso=ObtenEtiqueta(2387);
        }
    }


    $rowsa = RecuperaValor("SELECT notation_transcript FROM c_alumno WHERE fl_alumno=$fl_user ");
    ...
    if((empty($ds_foto))||($ds_foto=='null')){
        $ruta_imagen_instituto="fame/img/Partner_School_Logo.jpg";
    }else{
         $ruta_imagen_instituto="site/uploads/$fl_instituto/".$ds_foto;
    }
    #Recuperamos el template.
    $template  =GeneraTemplate(1,196);
    $template .=GeneraTemplate(2,196);
    $template .=GeneraTemplate(3,196);
    
    #Reemplazamos valores.
    
    $template =str_replace("#pg_delivery#",$fg_tipo_curso,$template);
    ...
    $template =str_replace("#institute_logo#",'./images/login-info.jpg',$template);
    $template=str_replace("#link_certificate#",$link_pdf,$template);
    $template=str_replace("#pg_stdate#",$st_inicio,$template);
    $template=str_replace("#pg_edate#",$fe_fin,$template);
    $template=str_replace("#iss_date#",$fe_emision,$template);
    $template=str_replace("#graduation_date#",$fe_graduacion,$template);
	$template=str_replace("#ds_campus#",$ds_campus,$template);

    $template = str_replace("#ds_notation#", $ds_notation, $template); ##notation diplomas and transcripts

if($fl_user == 10566){
        $template=str_replace("Download document",'Document',$template);
        $template=str_replace("#download_document#",'Unofficial Transcript',$template);
    }
    // is Diploma
    if($type == 1){
        $template=str_replace("#download_document#",'Diploma',$template);
    }
    // is transcript
    if($type == 2){
    ...
    // is transcript
    if($fl_user == 10566){
        $icono_pdf="<span style='font-size:30px;'><a href='../AD3M2SRC4/modules/reports/pctia_rpt.php?clave=".$fl_user."'><i class='fa fa-file-pdf-o' aria-hidden='true'></i> </a></span>";
        $template=str_replace("#icono_pdf#",'',$template);
    }

    // is Diploma
    if($type == 1){
        $icono_pdf="<span style='font-size:30px;'><a href='../AD3M2SRC4/modules/reports/newDiploma_rpt.php?clave=".$fl_user."'><i class='fa fa-file-pdf-o' aria-hidden='true'></i> </a></span>";
    ...
    <html lang="en-us">
        <head>
            ...
            <!-- #CSS Links -->
            <!-- Basic Styles -->
            <link rel="stylesheet" type="text/css" media="screen" href="fame/css/bootstrap.min.css">
            <link rel="stylesheet" type="text/css" media="screen" href="fame/css/font-awesome.min.css">
    
            <!-- SmartAdmin Styles : Caution! DO NOT change the order -->
            <link rel="stylesheet" type="text/css" media="screen" href="fame/css/smartadmin-production-plugins.min.css">
            ...
            <!-- #FAVICONS -->
            <link rel="shortcut icon" href="images/login-info.jpg" type="image/x-icon">
    ...
    <div id='main' role='main'>
            ...
            <div class="row">
                <div class="col-md-12">
                    <h1 class="page-title txt-color-blueDark text-center well" style="border:0px;background: none repeat 0 0 #2fabff;color:#fff;">						
                                                <?php echo 'Statement of VANAS Accreditation Status';?><br>
        ...
        <!-- PACE LOADER - turn this on if you want ajax loading to show (caution: uses lots of memory on iDevices)-->
            <script data-pace-options='{ "restartOnRequestAfter": true }' src="fame/js/plugin/pace/pace.min.js"></script>
    
            <!-- Link to Google CDN's jQuery + jQueryUI; fall back to local -->
            <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
            <script>
                if (!window.jQuery) {
                    document.write('<script src="fame/js/libs/jquery-3.2.1.min.js"><\/script>');
                }
            </script>
            ...
            <script>
                if (!window.jQuery.ui) {
                    document.write('<script src="fame/js/libs/jquery-ui.min.js"><\/script>');
                }
            </script>
    
            <!-- IMPORTANT: APP CONFIG -->
            <script src="fame/js/app.config.js"></script>
    
            <!-- JS TOUCH : include this plugin for mobile drag / drop touch events-->
            <script src="fame/js/plugin/jquery-touch/jquery.ui.touch-punch.min.js"></script> 
    
            <!-- BOOTSTRAP JS -->
            <script src="fame/js/bootstrap/bootstrap.min.js"></script>
    
            <!-- CUSTOM NOTIFICATION -->
            <script src="fame/js/notification/SmartNotification.min.js"></script>
    
            <!-- JARVIS WIDGETS -->
            <script src="fame/js/smartwidgets/jarvis.widget.min.js"></script>
    
            <!-- EASY PIE CHARTS -->
            <script src="fame/js/plugin/easy-pie-chart/jquery.easy-pie-chart.min.js"></script>
    
            <!-- SPARKLINES -->
            <script src="fame/js/plugin/sparkline/jquery.sparkline.min.js"></script>
    
            <!-- JQUERY VALIDATE -->
            <script src="fame/js/plugin/jquery-validate/jquery.validate.min.js"></script>
    
            <!-- JQUERY MASKED INPUT -->
            <script src="fame/js/plugin/masked-input/jquery.maskedinput.min.js"></script>
    
            <!-- JQUERY SELECT2 INPUT -->
            <script src="fame/js/plugin/select2/select2.min.js"></script>
    
            <!-- JQUERY UI + Bootstrap Slider -->
            <script src="fame/js/plugin/bootstrap-slider/bootstrap-slider.min.js"></script>
    
            <!-- browser msie issue fix -->
            <script src="fame/js/plugin/msie-fix/jquery.mb.browser.min.js"></script>
